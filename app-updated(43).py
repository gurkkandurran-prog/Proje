...
FLUID_LIBRARY = {
    ...
    "Steam": {
        ...
        "sg_func": None,
        "visc_func": None,
        "k_func": lambda t, p: calculate_specific_heat_ratio("Steam", t, p),
        "z_func": None,  # Steam doesn't use Z in the same way
        "pv_func": None,
        "rho_func": lambda t, p: calculate_density("Steam", t, p)
    },
    "CarbonDioxide": {
        "type": "gas",
    ...
    "Natural Gas": {
        ...
        "sg_func": lambda t, p: calculate_specific_gravity_gas("Natural Gas", t, p),
        "visc_func": None,
        "k_func": lambda t, p: calculate_specific_heat_ratio("Natural Gas", t, p),
        "z_func": lambda t, p: calculate_compressibility_factor("Natural Gas", t, p),
        "pv_func": None,
        "rho_func": lambda t, p: calculate_density("Natural Gas", t, p)
    },
    "Octane": {
        "type": "liquid",
    ...
    "Propane": {
        ...
        "sg_default": 0.507,  # Typical specific gravity at 15°C (density ~507 kg/m³)
        "sg_func": lambda t, p: calculate_specific_gravity_liquid("Propane", t, p),
...
def evaluate_valve_for_scenario_with_record(valve, scenario):
    ...
    # Calculate specific gravity based on fluid type and conditions
    fluid_library = scenario.get('fluid_library')
    ...
    if fluid_library in FLUID_LIBRARY:
        ...
        # Calculate specific gravity based on fluid type
        if fluid_type == "liquid":
            if fluid_data.get("sg_func"):
                sg_calculated = fluid_data["sg_func"](temp_c, p1_bar)
                scenario["sg"] = sg_calculated
                if record:
                    record.add_step(2, "Calculate specific gravity for liquid", 
                                   f"SG = ρ_fluid({temp_c}°C, {p1_bar}bar) / ρ_water(4°C)", 
                                   f"{sg_calculated:.4f}", "",
                                   {"Temperature": f"{temp_c}°C", "Pressure": f"{p1_bar} bar", 
                                    "Calculated SG": sg_calculated})
                    record.add_assumption(f"Specific gravity calculated from density at conditions", 
                                         "Dynamic calculation based on actual temperature and pressure",
                                         {"SG": sg_calculated, "Method": "ρ_fluid(T,P) / ρ_water(4°C)"})
            ...
            if fluid_data.get("sg_func"):
                ...
                scenario["sg"] = sg_calculated
                if record:
                    record.add_step(2, "Calculate specific gravity for gas", 
                                   "SG = M_fluid / M_air (at standard conditions)", 
                                   f"{sg_calculated:.4f}", "",
                                   {"Calculated SG": sg_calculated})
                    record.add_assumption(f"Specific gravity calculated from molecular weight ratio", 
                                         "Constant value based on molecular weight at standard conditions",
                                         {"SG": sg_calculated, "Method": "M_fluid / M_air"})
    ...
    for iteration in range(max_iterations):
        ...
        if record:
            record.add_step(3 + iteration, f"Iteration {iteration + 1} - Start", 
        ...
        if record and iteration == 0:
            record.add_step(4 + iteration, "Get valve characteristics at initial opening", 
        ...
        if record and iteration == 0:
            record.set_fp_details(fp_details)
            record.add_step(5 + iteration, "Calculate piping geometry factor Fp", 
                           "Fp = 1 / √(1 + (ΣK / N2) * (Cv/d²)²)", 
                           f"{fp:.4f}", "",
        ...
        if not use_valve_size:
            if fluid_type == "liquid":
                ...
                if record and iteration == 0:
                    record.add_step(6 + iteration, "Calculate combined liquid pressure recovery factor FLP", 
                ...
                if record and iteration == 0:
                    record.add_step(6 + iteration, "Calculate combined pressure drop ratio factor xTP", 
        ...
        if record:
            ...
            record.add_step(7 + iteration, f"Iteration {iteration + 1} - Results", 
        ...
        if abs(new_opening - opening) < tolerance:
            ...
            converged = True
            if record:
                record.add_step(8 + iteration, f"Iteration {iteration + 1} - Convergence", 
        ...
        if opening >= 100 or opening <= 0:
            converged = True
            if record:
                record.add_step(8 + iteration, f"Iteration {iteration + 1} - Boundary reached", 
    ...
    if fluid_library != "Select Fluid Library...":
        if fluid_library in FLUID_LIBRARY:
            ...
            if fluid_data["type"] == "liquid":
                if fluid_data.get("sg_func"):
                    st.session_state[f"sg_{scenario_num}"] = fluid_data["sg_func"](temp_c, p1_bar)
