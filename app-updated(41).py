...
# ========================
def get_water_density_at_std_conditions():
    """Get density of water at standard conditions (15°C, 1 atm)"""
    try:
        T_std = 15 + C_TO_K  # 15°C in Kelvin
        P_std = 1.01325 * 1e5  # 1 atm in Pa
        rho_water_std = CP.PropsSI('D', 'T', T_std, 'P', P_std, 'Water')
        return rho_water_std
    except:
        return 999.1  # kg/m³ at 15°C, 1 atm

def get_air_molecular_mass():
    """Get molecular mass of air"""
    try:
        M_air = CP.PropsSI('M', 'T', 273.15, 'P', 101325, 'Air')
        return M_air
    except:
        return 28.97  # g/mol

def get_fluid_molecular_mass(fluid_name):
    """Get molecular mass of a fluid using CoolProp"""
    try:
        if fluid_name in ["air", "Air"]:
            return get_air_molecular_mass()
        
        # Map fluid names to CoolProp names
        fluid_map = {
            "Methane": "Methane",
            "Hydrogen": "Hydrogen",
        ...
        coolprop_name = fluid_map.get(fluid_name, fluid_name)
        M = CP.PropsSI('M', 'T', 273.15, 'P', 101325, coolprop_name)
        return M
    except:
        # Return known molecular masses
        known_masses = {
            "Methane": 16.04,  # g/mol
            "Hydrogen": 2.016,  # g/mol
            "CarbonDioxide": 44.01,  # g/mol
            "Ammonia": 17.03,  # g/mol
            "Propane": 44.10,  # g/mol
            "Octane": 114.23,  # g/mol
            "Water": 18.015,  # g/mol
        ...
        return known_masses.get(fluid_name, 28.97)  # Default to air molecular mass

def calculate_specific_gravity(fluid_name, temp_c, pressure_bar, fluid_type):
    """
    Calculate specific gravity for fluids:
    - For liquids: SG = density(fluid, T, P) / density(water at 15°C, 1 atm)
    - For gases: SG = molecular_mass(fluid) / molecular_mass(air) [both at standard conditions]
    - For natural gas: constant 0.60
    - For jet fuels: empirical formula based on temperature and pressure
    """
    if fluid_name == "Natural Gas":
        return 0.60  # Constant value for natural gas
    
    if fluid_type == "liquid":
        if fluid_name in ["JP8", "JET A1"]:
            # Use jet fuel density formula
            density_fluid = calculate_density_jet_fuel(temp_c, pressure_bar, fluid_name)
        else:
            # Use general density calculation
            density_fluid = calculate_density(fluid_name, temp_c, pressure_bar)
        
        # Get water density at standard conditions
        density_water_std = get_water_density_at_std_conditions()
        
        if density_water_std > 0:
            return density_fluid / density_water_std
        else:
            return 1.0
    
    elif fluid_type in ["gas", "steam"]:
        if fluid_name == "20% Hydrogen + 80% Methane":
            # Calculate weighted average molecular mass
            M_H2 = get_fluid_molecular_mass("Hydrogen")
            M_CH4 = get_fluid_molecular_mass("Methane")
            M_mix = 0.2 * M_H2 + 0.8 * M_CH4
            M_air = get_air_molecular_mass()
            return M_mix / M_air
        else:
            # For pure gases
            M_fluid = get_fluid_molecular_mass(fluid_name)
            M_air = get_air_molecular_mass()
...
# ========================
# FLUID LIBRARY (UPDATED WITHOUT SG VALUES)
# ========================
FLUID_LIBRARY = {
    "Water": {
        "type": "liquid",
...
def calculate_density_jet_fuel(temp_c: float, press_bar: float, fuel_type: str) -> float:
    ...
    Uses empirical correlations based on temperature and pressure
    """
    # Base density at 15°C (kg/m³) and 1 atm
    if fuel_type == "JP8":
        base_density = 810.0  # kg/m³ at 15°C, 1 atm
    else:  # JET A1
        base_density = 800.0  # kg/m³ at 15°C, 1 atm
...
# ========================
# UPDATED FLUID PROPERTY UPDATE FUNCTIONS
# ========================
    ...
    if fluid_library != "Select Fluid Library...":
        if fluid_library in FLUID_LIBRARY:
            ...
            if fluid_type == "liquid":
                ...
                # Calculate specific gravity for liquids
                ...
                # Calculate specific gravity for gases
                sg = calculate_specific_gravity(fluid_library, temp_c, p1_bar, fluid_type)
                st.session_state[f"sg_{scenario_num}"] = sg
