...
# ========================
# UPDATED XTP CALCULATION FUNCTION (USES XT AT SPECIFIED OPENING)
# ========================
def calculate_x_tp(xt_at_op: float, valve_d_inch: float, pipe_d_inch: float, Fp: float, cv_at_op: float) -> float:
    """
    Calculate xTP (pressure drop ratio factor with reducers) using XT at actual operating point
    xt_at_op: XT value at the actual operating point (not 100% opening)
    cv_at_op: Cv value at the actual operating point
    """
    if abs(pipe_d_inch - valve_d_inch) < 0.01:
        return xt_at_op
    ...
    KB1 = 1 - d_ratio**4
    Ki = K1 + KB1
    term = 1 + (xt_at_op * Ki / CONSTANTS["N5"]["inch"]) * (cv_at_op / valve_d_inch**2)**2
...
def cv_gas(flow: float, p1: float, p2: float, sg: float, t: float, k: float, 
    ...
    fk = k / 1.4
    # FIXED: Use xt_at_op (which is XTP when reducers are present) for x_crit calculation
    x_crit = fk * xt_at_op
...
def evaluate_valve_for_scenario(valve, scenario):
    ...
    # ========================
    # ADDED: VALIDATION - PIPE CANNOT BE SMALLER THAN VALVE
    # ========================
    if pipe_d < valve_d:
        return {
            "op_point": 0,
            "req_cv": 0,
            "theoretical_cv": 0,
            "warning": "ERROR: Pipe diameter cannot be smaller than valve diameter!",
            "cavitation_info": "N/A",
            "status": "red",
            "margin": 0,
            "details": {"error": "Pipe diameter smaller than valve diameter"},
            "orifice_velocity": 0,
            "inlet_velocity": 0,
            "xt_valve": 0,
            "xtp": 0
    ...
    if scenario["fluid_type"] == "liquid":
        ...
        # Store XT values (for liquid, we don't use XT but we'll store them for consistency)
        xt_valve = valve.get_xt_at_opening(open_percent)
        xtp = xt_valve  # For liquid, XTP is same as XT (no reducers effect on XT for liquids)
        if abs(pipe_d - valve_d) > 0.01:
            # For liquid with reducers, we might need FLP instead
            flp = calculate_flp(valve, valve_d, pipe_d, cv_100)
            details['flp'] = flp
        ...
        for iteration in range(max_iterations):
            ...
            # ========================
            # UPDATED: Calculate XTP using XT at actual operating point, not 100%
            # ========================
            cv_at_op = valve.get_cv_at_opening(open_percent)
            if abs(pipe_d - valve_d) > 0.01:
                xt_at_op = calculate_x_tp(xt_valve, valve_d, pipe_d, fp, cv_at_op)
            else:
                xt_at_op = xt_valve
            ...
            cv_req, details = cv_gas(
                ...
                t=scenario["temp"],
                k=scenario["k"],
                xt_at_op=xt_at_op,  # This is XTP when reducers are present
                z=scenario["z"],
                fp=fp,
        ...
        # ========================
        # ADDED: Store both XT and XTP values
        # ========================
        xt_valve = valve.get_xt_at_opening(open_percent)
        cv_at_op = valve.get_cv_at_opening(open_percent)
        if abs(pipe_d - valve_d) > 0.01:
            xtp = calculate_x_tp(xt_valve, valve_d, pipe_d, fp, cv_at_op)
        else:
            xtp = xt_valve
            
        details['xt_valve'] = xt_valve
        details['xtp'] = xtp
        ...
        # Iterative approach for steam (similar to gas)
        max_iterations = 10
        tolerance = 0.1
        prev_op_point = 0
        open_percent = 100
    ...
    if 'error' in details:
        return {
            ...
            "xt_valve": details.get('xt_valve', 0),
            "xtp": details.get('xtp', 0)
        }
...
def scenario_input_form(scenario_num, selected_valve, scenario_data=None):
    default_values = {
        ...
        "rho": 1.0,
        "fluid_type": "liquid",
        "pipe_d": selected_valve.diameter if selected_valve else 2.0,  # FIXED: Default to valve diameter
    ...
    with col2:
        ...
        if not use_valve_size:
            # ========================
            # ADDED: VALIDATION MESSAGE FOR PIPE SIZE
            # ========================
            st.info("⚠️ **Note:** Pipe diameter should not be smaller than the selected valve diameter.")
            pipe_d = st.number_input(
                "Pipe Diameter (inch)", 
                min_value=0.1, 
                max_value=100.0, 
                value=scenario_data.get("pipe_d", selected_valve.diameter if selected_valve else 2.0), 
                step=0.1,
                key=f"pipe_d_{scenario_num}"
            )
        else:
            # FIXED: When checkbox is ticked, use valve diameter
            pipe_d = selected_valve.diameter if selected_valve else 2.0
...
def main():
    ...
    with tab1:
        scenario1 = scenario_input_form(1, selected_valve)
    with tab2:
        scenario2 = scenario_input_form(2, selected_valve)
    with tab3:
        scenario3 = scenario_input_form(3, selected_valve)
                ...
                with st.container():
                    ...
                    with st.expander(f"Detailed Calculations for {scenario['name']}"):
                        ...
                        if scenario["fluid_type"] in ["gas", "steam"]:
                            # ========================
                            # UPDATED: Show both XT and XTP values separately
                            # ========================
                            st.subheader("Pressure Drop Ratio Factors")
                            
                            # Get pipe diameter from scenario
                            pipe_d = scenario.get('pipe_d', selected_valve.diameter)
                            valve_d = selected_valve.diameter
                            
                            # Display XT (valve XT at operating point)
                            xt_valve = result.get('xt_valve', 0)
                            if isinstance(xt_valve, (int, float)):
                                st.markdown(f"**XT (Valve at {result['op_point']:.1f}% opening):** {xt_valve:.4f}")
                            else:
                                st.markdown(f"**XT (Valve at operating point):** {xt_valve}")
                            
                            # Display XTP (with reducers if applicable)
                            xtp = result.get('xtp', 0)
                            if isinstance(xtp, (int, float)):
                                if abs(pipe_d - valve_d) > 0.01:
                                    st.markdown(f"**XTP (with reducers):** {xtp:.4f}")
                                    st.caption(f"*Calculated with reducers: valve {valve_d}\" to pipe {pipe_d}\"*")
                                else:
                                    st.markdown(f"**XTP (same as XT):** {xtp:.4f}")
                                    st.caption("*No reducers - pipe diameter equals valve diameter*")
                            else:
                                st.markdown(f"**XTP:** {xtp}")
                            
                            # Add explanation about x_crit calculation
                            ...
                            if isinstance(x_crit_val, (int, float)):
                                ...
                                # Add note about x_crit calculation
                                if abs(pipe_d - valve_d) > 0.01:
                                    st.caption(f"*x_crit calculated using XTP = {xtp:.4f} (includes reducer effects)*")
                                else:
                                    st.caption(f"*x_crit calculated using XT = {xt_valve:.4f} (no reducers)*")
                            else:
