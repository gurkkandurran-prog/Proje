...
class CalculationRecord:
    ...
    def set_input_parameter(self, key, value, unit=""):
        """Set an input parameter"""
        self.input_parameters[key] = {'value': value, 'unit': unit}
    ...
    try:
        ...
        for i, scenario in enumerate(scenarios):
            ...
            scenario_data = {
                ...
                "Inlet Pressure": {"value": f"{scenario['p1']:.2f}", "unit": "bar"},
                "Outlet Pressure": {"value": f"{scenario['p2']:.2f}", "unit": "bar"},
                "Temperature": {"value": f"{scenario['temp']:.1f}", "unit": "°C"},
                "Inlet Pipe Diameter": {"value": f"{scenario['pipe_d_in']}", "unit": "inch"},
                "Outlet Pipe Diameter": {"value": f"{scenario['pipe_d_out']}", "unit": "inch"}
            }
...
def evaluate_valve_for_scenario_with_record(valve, scenario):
    ...
    for key, value in scenario.items():
        if isinstance(value, (int, float, str)):
            record.set_input_parameter(key.replace('_', ' ').title(), value, "")
    ...
    for iteration in range(max_iterations):
        ...
        if record and iteration == 0:
            ...
            record.add_formula(
                ...
                {"K1": "Inlet reducer coefficient", "K2": "Outlet reducer coefficient", 
                 "KB1": "Inlet Bernoulli coefficient", "KB2": "Outlet Bernoulli coefficient"}
            )
    ...
    if record:
        record.add_step(101, "Calculate flow velocities", 
                       f"Orifice area based on Cv ratio: {final_cv}/{valve.get_cv_at_opening(100)}", 
