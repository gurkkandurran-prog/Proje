...
def calculate_flp(valve, valve_d_inch: float, pipe_d_inch: float, open_percent: float) -> float:
    """
    Calculate FLP at specific opening percentage
    Uses Fl and Cv at the operating point
    """
    if abs(pipe_d_inch - valve_d_inch) < 0.01:
        return valve.get_fl_at_opening(open_percent)
    
    # Get characteristics at OPERATING POINT
    ...
    # Use cv_at_op (operating point) not cv_100!
    term = (Ki / CONSTANTS["N2"]["inch"]) * (cv_at_op / valve_d_inch**2)**2 + 1/fl_at_op**2
    FLP = 1 / math.sqrt(term)
    return FLP

def calculate_x_tp(valve, valve_d_inch: float, pipe_d_inch: float, Fp: float, open_percent: float) -> float:
    """
    Calculate XTP (pressure drop ratio factor with piping geometry) at specific opening
    
    According to IEC 60534-2-1 / ISA-75.01.01:
    
    For reducers/expanders attached to valve:
    xTP = xT / Fp² * [1 / (1 + (xT * Ki) / N5 * (Cv/d²)²)]
    
    Where:
    - xT: Pressure drop ratio factor at current opening
    - Cv: Flow coefficient at current opening
    - d: Valve internal diameter (inches)
    - Ki: Sum of inlet and outlet velocity head coefficients
    - N5: Constant (1000 for inch units)
    - Fp: Piping geometry factor
    
    Args:
        valve: Valve object
        valve_d_inch: Valve internal diameter (inches)
        pipe_d_inch: Pipe internal diameter (inches)
        Fp: Piping geometry factor (calculated with Cv at 100%)
        open_percent: Opening percentage (0-100)
    
    Returns:
        xTP: Corrected pressure drop ratio factor for piping
    """
    # If no reducers/expanders (pipe same size as valve)
    if abs(pipe_d_inch - valve_d_inch) < 0.01:
        return valve.get_xt_at_opening(open_percent)
    
    # Get valve characteristics at CURRENT operating point
    xT = valve.get_xt_at_opening(open_percent)  # <-- Xt at actual opening
    cv_at_op = valve.get_cv_at_opening(open_percent)  # <-- Cv at actual opening
    
    # Calculate diameter ratio
    d_ratio = valve_d_inch / pipe_d_inch
    
    # Inlet head loss coefficient for reducer
    K1 = 0.5 * (1 - d_ratio**2)**2
    
    # Bernoulli coefficient for reducer
    KB1 = 1 - d_ratio**4
    
    # Sum of coefficients
    Ki = K1 + KB1
    
    # Get N5 constant for inch units
    N5 = CONSTANTS["N5"]["inch"]  # 1000
    
    # Calculate term: 1 + (xT * Ki / N5) * (Cv/d²)²
    cv_d2_ratio = cv_at_op / (valve_d_inch**2)
    term = 1 + (xT * Ki / N5) * (cv_d2_ratio**2)
    ...
    if open_percent != 100:
        print(f"XTP Debug: open={open_percent}%, xT={xT:.4f}, Cv={cv_at_op:.1f}, "
              f"d_ratio={d_ratio:.3f}, Ki={Ki:.3f}, term={term:.4f}, xTP={xTP:.4f}")
