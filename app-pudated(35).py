...
class CalculationRecord:
    ...
    def add_step(self, step_number, description, formula, result, unit=""):
        """Add a calculation step to the record"""
        self.steps.append({
            'step': step_number,
    ...
    def add_formula(self, name, formula, explanation):
    ...
    def add_assumption(self, assumption, reason):
    ...
    def add_warning(self, warning, severity="info"):
    ...
    def set_result(self, key, value, unit=""):
    ...
    def generate_report_text(self):
        """Generate a text report of the calculation"""
        report = []
        report.append(f"Calculation Report for {self.scenario_name}")
        report.append(f"Fluid Type: {self.fluid_type}")
        report.append("=" * 50)
        report.append("\nFORMULAS USED:")
        report.append("-" * 30)
...
def calculate_piping_factor_fp_separate(valve_d_inch: float, pipe_d_in_inch: float, 
                                       pipe_d_out_inch: float, cv_op: float) -> float:
    """
    ...
    # Calculate Fp
    term = 1 + (sumK / CONSTANTS["N2"]["inch"]) * (cv_op / valve_d_inch**2)**2
    Fp = 1 / math.sqrt(term)
    
    return Fp

def calculate_flp_separate(valve, valve_d_inch: float, pipe_d_in_inch: float, 
                          pipe_d_out_inch: float, cv_op: float, fl_at_op: float) -> float:
    """
    ...
    # Calculate outlet reducer/expander coefficient K2 (not used in FLP)
    # K2 = 1.0 * (1 - d_ratio_out**2)**2 if d_ratio_out < 1 else 0
    ...
    term = (Ki / CONSTANTS["N2"]["inch"]) * (cv_op / valve_d_inch**2)**2 + 1/fl_at_op**2
    FLP = 1 / math.sqrt(term)
    
    return FLP

def calculate_x_tp_separate(valve, valve_d_inch: float, pipe_d_in_inch: float, 
                           pipe_d_out_inch: float, fp: float, cv_op: float, xt_at_op: float) -> float:
    """
    ...
    term = 1 + (xt_at_op * Ki / CONSTANTS["N5"]["inch"]) * (cv_op / valve_d_inch**2)**2
    xTP = xt_at_op / fp**2 * (1 / term)
    
    return xTP

def calculate_velocity_pressure_drop(flow_m3h: float, pipe_d_inch: float, density_kg_m3: float) -> tuple:
    """
    Calculate velocity and velocity pressure drop in pipe
    Returns: (velocity_m_s, pressure_drop_bar)
    """
    # Convert pipe diameter to meters
    d_m = pipe_d_inch * 0.0254
    ...
    # Calculate velocity pressure drop (dynamic pressure)
    # ΔP = 0.5 * ρ * v²
    pressure_drop_pa = 0.5 * density_kg_m3 * velocity**2
    pressure_drop_bar = pressure_drop_pa * 1e-5
    
    return velocity, pressure_drop_bar

# ========================
# CV_LIQUID FUNCTION WITH SEPARATE PIPE SIZES AND CALCULATION RECORD
# ========================
def cv_liquid_with_record(flow: float, p1: float, p2: float, sg: float, fl_at_op: float, 
    ...
    """
    Calculate required Cv for liquid with calculation record
    """
    if record:
        record.add_formula(
    ...
    if record:
        record.add_step(1, "Calculate pressure drop", "ΔP = P1 - P2", f"{dp:.3f}", "bar")
    
    if dp <= 0:
        if record:
            record.add_warning("No pressure drop (ΔP <= 0)", "error")
    ...
    if record:
        record.add_step(4, "Calculate maximum valve pressure drop", 
                       "ΔP_max_valve = Fl² * (P1 - FF * Pv)", f"{dp_max_valve:.3f}", "bar")
    ...
    if dp < dp_max_valve:
        ...
        if record:
            record.add_step(5, "Calculate pseudo Cv (unchoked)", 
                           "Cv_pseudo = (Q / N1) * √(SG / ΔP)", f"{cv_pseudo:.2f}", "")
    else:
    ...
    if record:
        record.add_step(7, "Calculate viscosity correction factor Fr", 
                       "Fr = f(Re) from IEC table", f"{fr:.4f}", "")
        if rev < 40000:
            record.add_warning(f"Low Reynolds number ({rev:.0f}), applying viscosity correction", "info")
    ...
    if record:
        record.add_step(8, "Apply piping correction Fp", "Cv_after_Fp = Cv_pseudo / Fp", 
                       f"{cv_after_fp:.2f}", "")
        record.add_step(9, "Apply viscosity correction Fr", "Cv_corrected = Cv_after_Fp / Fr", 
                       f"{corrected_cv:.2f}", "")
        record.set_result("Required Cv", corrected_cv, "")
        record.set_result("Theoretical Cv", theoretical_cv, "")
        record.set_result("Reynolds Number", rev, "")
        record.set_result("Viscosity Correction Fr", fr, "")
        record.set_result("Critical Pressure Ratio FF", ff, "")
...
# ========================
# CV_GAS FUNCTION WITH SEPARATE PIPE SIZES AND CALCULATION RECORD
# ========================
def cv_gas_with_record(flow: float, p1: float, p2: float, sg: float, t: float, k: float, 
    ...
    """
    Calculate required Cv for gas with calculation record
    """
    if record:
        record.add_formula(
            "Gas Cv Calculation",
    ...
    x_actual = (p1 - p2) / p1
    if record:
        record.add_step(1, "Calculate pressure drop ratio", "X = (P1 - P2) / P1", 
                       f"{x_actual:.4f}", "")
    ...
    if record:
        record.add_step(2, "Calculate k ratio", "Fk = k / 1.4", f"{fk:.4f}", "")
        record.add_step(3, "Calculate critical pressure drop ratio", 
                       "X_crit = Fk * xT", f"{x_crit:.4f}", "")
    ...
    if x_actual >= x_crit:
        ...
        is_choked = True
        if record:
            record.add_step(4, "Flow is choked, use X_crit", "X_actual ≥ X_crit", 
    ...
    if record:
        ...
        record.set_result("Required Cv", corrected_cv, "")
        record.set_result("Theoretical Cv", theoretical_cv, "")
        record.set_result("Expansion Factor Y", y, "")
        record.set_result("Pressure Drop Ratio X", x_actual, "")
        record.set_result("Critical X", x_crit, "")
...
# ========================
# CV_STEAM FUNCTION WITH SEPARATE PIPE SIZES AND CALCULATION RECORD
# ========================
def cv_steam_with_record(flow: float, p1: float, p2: float, rho: float, k: float, 
                        xt_at_op: float, fp: float = 1.0, op_point: float = None,
                        cv_op: float = None, record: CalculationRecord = None) -> tuple:
    """
    Calculate required Cv for steam with calculation record
    """
    if record:
        record.add_formula(
    ...
    if record:
        record.add_step(6, "Calculate theoretical Cv", 
                       "Cv_theo = Q / (N6 * Y * √(Term))", f"{theoretical_cv:.2f}", "")
        record.add_step(7, "Apply piping correction Fp", 
                       "Cv_corrected = Cv_theo / Fp", f"{corrected_cv:.2f}", "")
        record.set_result("Required Cv", corrected_cv, "")
        record.set_result("Theoretical Cv", theoretical_cv, "")
        record.set_result("Expansion Factor Y", y, "")
        record.set_result("Pressure Drop Ratio X", x_actual, "")
...
def check_cavitation(p1: float, p2: float, pv: float, fl_at_op: float, pc: float, 
                    record: CalculationRecord = None) -> tuple:
    """
    Check cavitation with calculation record
    """
    if pc <= 0:
        if record:
    ...
    if record:
        record.add_step(2, "Calculate sigma (cavitation index)", 
                       "σ = (P1 - Pv) / ΔP", f"{sigma:.2f}", "")
        record.add_step(3, "Calculate valve recovery coefficient", 
                       "Km = Fl²", f"{km:.2f}", "")
...
def generate_simple_pdf_report(scenarios, valve, op_points, req_cvs, warnings, cavitation_info,
    ...
    try:
        ...
        specs = [
            ...
            ("Max Kv", f"{cv_to_kv(valve.get_cv_at_opening(100)):.1f}"),
        ...
        pdf.add_table(results_headers, results_data, [30, 30, 20, 30, 20, 30])
        ...
        for i, scenario in enumerate(scenarios):
            ...
            pdf.add_key_value("Flow Rate", f"{scenario['flow_display']:.1f} {scenario['flow_unit']}")
            pdf.add_key_value("Equivalent Flow", f"{scenario['flow']:.1f} {flow_unit}")
            ...
            pdf.add_key_value("Inlet Pipe Diameter", f"{scenario['pipe_d_in']} inch")
            pdf.add_key_value("Outlet Pipe Diameter", f"{scenario['pipe_d_out']} inch")
            ...
            pdf.add_key_value("Required Kv", f"{req_kv:.1f}")
            pdf.add_key_value("Valve Cv at Opening", f"{actual_cv:.1f}")
            pdf.add_key_value("Valve Kv at Opening", f"{actual_kv:.1f}")
            ...
            # Add calculation record if available
            if calculation_records and i < len(calculation_records) and calculation_records[i]:
                pdf.add_page()
                pdf.add_subtitle(f"Calculation Record: {scenario['name']}")
                
                record = calculation_records[i].get_summary()
                pdf.add_paragraph(f"Total Steps: {record['total_steps']}")
                pdf.add_paragraph(f"Total Formulas: {record['total_formulas']}")
                
                if record['assumptions']:
                    pdf.add_subtitle("Assumptions")
                    for assumption in record['assumptions']:
                        pdf.add_paragraph(f"• {assumption['assumption']} ({assumption['reason']})")
                
                if record['warnings']:
                    pdf.add_subtitle("Warnings")
                    for warning in record['warnings']:
                        severity = warning['severity'].upper()
                        pdf.add_paragraph(f"[{severity}] {warning['warning']}")
...
# ========================
# VELOCITY CALCULATION WITH SEPARATE PIPE SIZES
# ========================
def calculate_valve_velocity_separate(scenario, valve, op_point):
    ...
    if scenario["fluid_type"] == "liquid":
        flow_m3h = scenario["flow"]  # Already in m³/h
        flow_m3s = flow_m3h / 3600
        density = scenario.get('density', 1000)  # Approximate if not available
    elif scenario["fluid_type"] == "gas":
        flow_std_m3h = scenario["flow"]  # Already in std m³/h
        T_actual = scenario["temp"] + C_TO_K
        P_actual = scenario["p1"] * 1e5
        T_std = 288.15
        P_std = 101325
        Q_actual = flow_std_m3h * (P_std / P_actual) * (T_actual / T_std) * scenario["z"]
        flow_m3s = Q_actual / 3600
        density = scenario.get('density', 1.2)  # Approximate air density
    else:  # steam
        flow_kg_h = scenario["flow"]  # Already in kg/h
        density = scenario["rho"]
        volume_flow = flow_kg_h / density
        flow_m3s = volume_flow / 3600
    
    # Calculate velocities
    orifice_velocity = flow_m3s / area_op if area_op > 0 else 0
    
    # Calculate inlet and outlet pipe velocities
    inlet_d_m = scenario["pipe_d_in"] * 0.0254
    outlet_d_m = scenario["pipe_d_out"] * 0.0254
    
    inlet_area = math.pi * (inlet_d_m / 2)**2
    outlet_area = math.pi * (outlet_d_m / 2)**2
    
    inlet_velocity = flow_m3s / inlet_area if inlet_area > 0 else 0
    outlet_velocity = flow_m3s / outlet_area if outlet_area > 0 else 0
    
    # Calculate velocity pressure drops
    inlet_pressure_drop = 0.5 * density * inlet_velocity**2 * 1e-5  # bar
    outlet_pressure_drop = 0.5 * density * outlet_velocity**2 * 1e-5  # bar
...
# ========================
# UPDATED EVALUATE_VALVE_FOR_SCENARIO WITH SEPARATE PIPE SIZES AND CALCULATION RECORD
# ========================
def evaluate_valve_for_scenario_with_record(valve, scenario):
    """
    Evaluate valve for scenario with separate inlet/outlet pipe sizes and calculation record
    """
    ...
    for iteration in range(max_iterations):
        ...
        if record and iteration == 0:
            record.add_step(2, "Get valve characteristics at initial opening", 
                           f"Opening: {opening}%, Cv: {cv_at_op:.1f}, Fl: {fl_at_op:.3f}, xT: {xt_at_op:.4f}", 
                           "Valve characteristics retrieved", "")
        
        # Calculate Fp using separate inlet/outlet pipe diameters
        fp = calculate_piping_factor_fp_separate(valve_d, pipe_d_in, pipe_d_out, cv_at_op)
        
        if record and iteration == 0:
            record.add_step(3, "Calculate piping geometry factor Fp", 
                           "Fp = 1 / √(1 + (ΣK / N2) * (Cv/d²)²)", 
                           f"{fp:.4f}", "")
            record.add_formula(
                "Piping Geometry Factor Fp",
                "Fp = 1 / √(1 + (ΣK / N2) * (Cv/d²)²) where ΣK = K1 + K2 + KB1 - KB2",
                "Accounts for pressure loss due to pipe reducers/expanders"
            )
        ...
        if not use_valve_size:
            if fluid_type == "liquid":
                fl_effective = calculate_flp_separate(valve, valve_d, pipe_d_in, pipe_d_out, cv_at_op, fl_at_op)
                if record and iteration == 0:
                    record.add_step(4, "Calculate combined liquid pressure recovery factor FLP", 
                                   "FLP = 1 / √((Ki/N2)*(Cv/d²)² + 1/Fl²)", 
                                   f"{fl_effective:.4f}", "")
            elif fluid_type in ["gas", "steam"]:
                xt_effective = calculate_x_tp_separate(valve, valve_d, pipe_d_in, pipe_d_out, fp, cv_at_op, xt_at_op)
                if record and iteration == 0:
                    record.add_step(4, "Calculate combined pressure drop ratio factor xTP", 
    ...
    final_fp = calculate_piping_factor_fp_separate(valve_d, pipe_d_in, pipe_d_out, final_cv)
    ...
    if not use_valve_size:
        if fluid_type == "liquid":
            final_fl_effective = calculate_flp_separate(valve, valve_d, pipe_d_in, pipe_d_out, final_cv, final_fl)
        elif fluid_type in ["gas", "steam"]:
            final_xt_effective = calculate_x_tp_separate(valve, valve_d, pipe_d_in, pipe_d_out, final_fp, final_cv, final_xt)
    ...
    # Calculate velocities with separate pipe sizes
    ...
    if record:
        record.set_result("Final Opening", opening, "%")
        record.set_result("Final Cv", cv_valve_final, "")
        record.set_result("Final Kv", kv_valve_final, "")
        record.set_result("Required Cv", cv_req_final, "")
        ...
        if not use_valve_size:
            ...
            with col_pipe1:
                pipe_d_in = st.number_input(
                    "Inlet Pipe Diameter (inch)", 
                    min_value=0.1, 
                    max_value=100.0, 
                    value=scenario_data["pipe_d_in"], 
                    step=0.1,
                    key=f"pipe_d_in_{scenario_num}"
                )
            with col_pipe2:
                pipe_d_out = st.number_input(
                    "Outlet Pipe Diameter (inch)", 
                    min_value=0.1, 
                    max_value=100.0, 
                    value=scenario_data["pipe_d_out"], 
                    step=0.1,
                    key=f"pipe_d_out_{scenario_num}"
                )
            st.caption(f"Inlet/Outlet pipe ratio: {pipe_d_in/pipe_d_out:.2f} (in/out)")
        else:
...
def main():
    ...
    with col2:
        ...
        st.markdown("*Now with separate inlet/outlet pipe sizes and calculation records*")
    ...
    if calculate_btn:
        ...
        try:
            ...
            st.success("Calculation completed with separate pipe sizes!")
        except Exception as e:
            ...
            for i, scenario in enumerate(scenarios):
                ...
                with st.container():
                    ...
                    with st.expander(f"Detailed Calculations for {scenario['name']}"):
                        ...
                        col1, col2 = st.columns(2)
                        with col1:
                            st.markdown(f"**Inlet Pipe Diameter:** {scenario['pipe_d_in']} inch")
                            st.markdown(f"**Outlet Pipe Diameter:** {scenario['pipe_d_out']} inch")
                        ...
                        st.markdown(f"**Orifice Velocity:** {result.get('orifice_velocity', 0):.2f} m/s")
                        st.markdown(f"**Inlet Pipe Velocity:** {result.get('inlet_velocity', 0):.2f} m/s")
                        st.markdown(f"**Outlet Pipe Velocity:** {result.get('outlet_velocity', 0):.2f} m/s")
                        st.markdown(f"**Inlet Velocity Pressure Drop:** {result.get('inlet_pressure_drop', 0):.4f} bar")
                        st.markdown(f"**Outlet Velocity Pressure Drop:** {result.get('outlet_pressure_drop', 0):.4f} bar")
                        ...
                        # Show calculation record
                        if calculation_records and i < len(calculation_records) and calculation_records[i]:
                            with st.expander("View Calculation Record", expanded=False):
                                record = calculation_records[i]
                                st.markdown("### Calculation Steps and Formulas")
                                
                                # Create a download button for the calculation record
                                record_text = record.generate_report_text()
                                st.download_button(
                                    label="Download Calculation Record",
                                    data=record_text,
                                    file_name=f"Calculation_Record_{scenario['name']}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt",
                                    mime="text/plain"
                                )
                                ...
                                for formula in record.formulas:
                                    with st.expander(f"Formula: {formula['name']}"):
                                        ...
                                        st.markdown(f"*{formula['explanation']}*")
                                ...
                                if record.assumptions:
                                    ...
                                    for assumption in record.assumptions:
                                        ...
                                        st.markdown(f"  *Reason: {assumption['reason']}*")
                                ...
                                if record.warnings:
                                    ...
                                    for warning in record.warnings:
                                        ...
                                        st.markdown(f"- <span style='color:{severity_color};'>[{warning['severity'].upper()}]</span> {warning['warning']}", unsafe_allow_html=True)
