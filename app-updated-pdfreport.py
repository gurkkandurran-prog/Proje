...
# Velocity limits (m/s) – updated according to user requirements
LIQUID_VELOCITY_LIMITS = {"inlet_outlet": 2.0, "orifice": 9.0}
GAS_VELOCITY_LIMITS = {"inlet_outlet": 15.0, "orifice_mach": 0.7}
STEAM_VELOCITY_LIMITS = {"inlet_outlet": 15.0, "orifice": 15.0}   # orifice limit unchanged
...
def calculate_valve_velocity_separate(scenario, valve, op_point):
    ...
    # --- Velocity warning logic using new limits ---
    warnings = []
    
    if fluid_type == "liquid":
        # Liquid limits: inlet/outlet 2 m/s, orifice 9 m/s
        if inlet_velocity > LIQUID_VELOCITY_LIMITS["inlet_outlet"]:
            warnings.append(f"High inlet velocity ({inlet_velocity:.1f} m/s) for liquid! (max {LIQUID_VELOCITY_LIMITS['inlet_outlet']} m/s)")
        if outlet_velocity > LIQUID_VELOCITY_LIMITS["inlet_outlet"]:
            warnings.append(f"High outlet velocity ({outlet_velocity:.1f} m/s) for liquid! (max {LIQUID_VELOCITY_LIMITS['inlet_outlet']} m/s)")
        if orifice_velocity > LIQUID_VELOCITY_LIMITS["orifice"]:
            warnings.append(f"High orifice velocity ({orifice_velocity:.1f} m/s) for liquid! (max {LIQUID_VELOCITY_LIMITS['orifice']} m/s)")
    
    elif fluid_type == "gas":
        # Gas limits: inlet/outlet 15 m/s, orifice limited by Mach number
        if inlet_velocity > GAS_VELOCITY_LIMITS["inlet_outlet"]:
            warnings.append(f"High inlet velocity ({inlet_velocity:.1f} m/s) for gas! (max {GAS_VELOCITY_LIMITS['inlet_outlet']} m/s)")
        if outlet_velocity > GAS_VELOCITY_LIMITS["inlet_outlet"]:
            warnings.append(f"High outlet velocity ({outlet_velocity:.1f} m/s) for gas! (max {GAS_VELOCITY_LIMITS['inlet_outlet']} m/s)")
        
        # Compute speed of sound for gas at orifice conditions
        # a = sqrt(k * R_specific * T)
        # R_specific = R_universal / (sg * M_air)   with R_universal = 8314.462618 J/(kmol·K), M_air = 28.97 kg/kmol
        # For simplicity we use R_specific = 287.05 * (1 / sg?) Actually for air (sg=1) R_specific = 287 J/(kg·K)
        # More generally: R_specific = 8314.462618 / (sg * 28.97)   (units: J/(kg·K))
        sg = scenario.get("sg", 1.0)
        if sg > 0:
            R_specific = 8314.462618 / (sg * 28.97)   # J/(kg·K)
        else:
            R_specific = 287.05   # fallback for air
        k = scenario.get("k", 1.4)
        T_orifice_K = (p_avg / p1) * (temp + C_TO_K)   # approximate temperature at orifice (isenthalpic)
        # Use actual temperature if not too far
        if T_orifice_K < 100:
            T_orifice_K = temp + C_TO_K
        sound_speed = math.sqrt(k * R_specific * T_orifice_K)
        mach = orifice_velocity / sound_speed if sound_speed > 0 else 0.0
        if mach > GAS_VELOCITY_LIMITS["orifice_mach"]:
            warnings.append(f"High orifice Mach number ({mach:.3f}) for gas! (max {GAS_VELOCITY_LIMITS['orifice_mach']}) (velocity={orifice_velocity:.1f} m/s, sound speed={sound_speed:.1f} m/s)")
    
    elif fluid_type == "steam":
        # Steam limits: inlet/outlet 15 m/s, orifice also limited to 15 m/s (or could be extended)
        if inlet_velocity > STEAM_VELOCITY_LIMITS["inlet_outlet"]:
            warnings.append(f"High inlet velocity ({inlet_velocity:.1f} m/s) for steam! (max {STEAM_VELOCITY_LIMITS['inlet_outlet']} m/s)")
        if outlet_velocity > STEAM_VELOCITY_LIMITS["inlet_outlet"]:
            warnings.append(f"High outlet velocity ({outlet_velocity:.1f} m/s) for steam! (max {STEAM_VELOCITY_LIMITS['inlet_outlet']} m/s)")
        if orifice_velocity > STEAM_VELOCITY_LIMITS["orifice"]:
            warnings.append(f"High orifice velocity ({orifice_velocity:.1f} m/s) for steam! (max {STEAM_VELOCITY_LIMITS['orifice']} m/s)")
    ...
    return {
        ...
        'mach' if fluid_type == "gas" else None: mach if fluid_type == "gas" else None,
        'sound_speed' if fluid_type == "gas" else None: sound_speed if fluid_type == "gas" else None
    }
                ...
                with st.container():
                    ...
                    with st.expander(f"Detailed Calculations for {scenario['name']}", expanded=False):
                        ...
                        with col_vel3:
                            ...
                            if scenario["fluid_type"] == "gas":
                                mach = velocity_details.get('mach', 0)
                                sound = velocity_details.get('sound_speed', 0)
                                if mach and sound:
                                    st.markdown(f"Mach number: {mach:.3f} (Speed of sound: {sound:.1f} m/s)")
