...
def calculate_piping_factor_fp(valve_d_inch: float, pipe_d_inch: float, cv_op: float) -> float:
    """
    Calculate piping geometry factor Fp using Cv at operating point
    """
...
def calculate_flp(valve, valve_d_inch: float, pipe_d_inch: float, cv_op: float, fl_at_op: float) -> float:
    """
    Calculate combined liquid pressure recovery factor FLP using Cv at operating point
    """
...
def calculate_x_tp(valve, valve_d_inch: float, pipe_d_inch: float, fp: float, cv_op: float, xt_at_op: float) -> float:
    """
    Calculate combined pressure drop ratio factor xTP using Cv at operating point
    """
    if abs(pipe_d_inch - valve_d_inch) < 0.01:
        return xt_at_op
    d_ratio = valve_d_inch / pipe_d_inch
...
# ========================
# CV_LIQUID FUNCTION WITH FULL ITERATIVE SUPPORT
# ========================
def cv_liquid(flow: float, p1: float, p2: float, sg: float, fl_at_op: float, 
              pv: float, pc: float, visc_cst: float, d_m: float, 
              valve, fp: float = 1.0, cv_op: float = None) -> tuple:
    """
    Calculate required Cv for liquid with iterative Fp calculation
    cv_op: Cv at operating point (for Fp calculation)
    """
    ...
    details = {
        ...
        'dp_max': dp_max_valve,
        'fl_at_op': fl_at_op,
        'cv_used_for_fp': cv_op  # Store which Cv was used for Fp calculation
    }
    
    return corrected_cv, details

# ========================
# CV_GAS FUNCTION WITH FULL ITERATIVE SUPPORT
# ========================
def cv_gas(flow: float, p1: float, p2: float, sg: float, t: float, k: float, 
           xt_at_op: float, z: float, fp: float = 1.0, op_point: float = None,
           cv_op: float = None) -> tuple:
    """
    Calculate required Cv for gas with iterative Fp calculation
    cv_op: Cv at operating point (for Fp calculation)
    """
    if p1 <= 0 or p2 < 0 or p1 <= p2:
...
# ========================
# CV_STEAM FUNCTION WITH FULL ITERATIVE SUPPORT
# ========================
def cv_steam(flow: float, p1: float, p2: float, rho: float, k: float, 
             xt_at_op: float, fp: float = 1.0, op_point: float = None,
             cv_op: float = None) -> tuple:
    """
    Calculate required Cv for steam with iterative Fp calculation
    cv_op: Cv at operating point (for Fp calculation)
    """
    if p1 <= 0 or p2 < 0 or p1 <= p2:
        return 0, {'error': 'Invalid pressures', 'theoretical_cv': 0, 'fp': fp, 'expansion_factor': 0, 'is_choked': False, 'x_crit': 0, 'x_actual': 0, 'xt_at_op': xt_at_op, 'xt_op_point': op_point}
...
# ========================
# UPDATED EVALUATE_VALVE_FOR_SCENARIO WITH COMPLETE ITERATION AND FINAL ITERATION RECORDING
# ========================
def evaluate_valve_for_scenario(valve, scenario):
    """
    Evaluate valve for scenario with COMPLETE iteration of ALL valve characteristics
    INCLUDES recording of final iteration
    """
    ...
    tolerance = 0.5  # 0.5% tolerance for opening convergence (more strict)
    prev_opening = 0
    opening = 50  # Start with 50% opening
    converged = False
    iteration_results = []
    
    for iteration in range(max_iterations):
        # 1. Get ALL valve characteristics at CURRENT opening
        cv_at_op = valve.get_cv_at_opening(opening)
        fl_at_op = valve.get_fl_at_opening(opening)
        xt_at_op = valve.get_xt_at_opening(opening)
        
        # 2. Calculate Fp using Cv at current operating point
        fp = calculate_piping_factor_fp(valve_d, pipe_d, cv_at_op)
        
        # 3. Apply piping corrections if pipe diameter differs
        fl_effective = fl_at_op
        xt_effective = xt_at_op
        
        if not use_valve_size:
            if fluid_type == "liquid":
                # Calculate FLP (combined liquid pressure recovery factor)
                fl_effective = calculate_flp(valve, valve_d, pipe_d, cv_at_op, fl_at_op)
            elif fluid_type in ["gas", "steam"]:
                # Calculate xTP (combined pressure drop ratio factor)
                xt_effective = calculate_x_tp(valve, valve_d, pipe_d, fp, cv_at_op, xt_at_op)
        
        # 4. Calculate required Cv with updated factors
        if fluid_type == "liquid":
            ...
            cv_req, details = cv_liquid(
                ...
                p2=scenario["p2"],
                sg=scenario["sg"],
                fl_at_op=fl_effective,  # Use effective Fl
                pv=scenario["pv"],
                pc=scenario["pc"],
            ...
            cv_req, details = cv_gas(
                ...
                t=scenario["temp"],
                k=scenario["k"],
                xt_at_op=xt_effective,  # Use effective Xt
                z=scenario["z"],
                fp=fp,
                op_point=opening,
                cv_op=cv_at_op
        ...
        # 5. Find new opening that provides at least the required Cv
        new_opening = 0
        cv_valve = 0
        
        # Binary search for optimal opening (faster and more accurate)
        low, high = 0, 100
        for _ in range(10):  # Binary search for up to 10 iterations
            mid = (low + high) / 2
        ...
        # If binary search didn't find exact, do linear search from found point
        ...
        if abs(new_opening - opening) < tolerance:
            opening = new_opening
            converged = True
            # Don't break yet - we'll record the final iteration after the loop
            break
        
        # Update for next iteration
        opening = new_opening
        
        # Early exit if we're at a boundary
    ...
    final_fp = calculate_piping_factor_fp(valve_d, pipe_d, final_cv)
    ...
    if not use_valve_size:
        if fluid_type == "liquid":
            final_fl_effective = calculate_flp(valve, valve_d, pipe_d, final_cv, final_fl)
        elif fluid_type in ["gas", "steam"]:
            final_xt_effective = calculate_x_tp(valve, valve_d, pipe_d, final_fp, final_cv, final_xt)
    ...
    if fluid_type == "liquid":
        ...
        if scenario["pc"] > 0:
            choked, sigma, km, cav_msg = check_cavitation(
                scenario["p1"], scenario["p2"], scenario["pv"], final_fl_effective, scenario["pc"]
            )
    ...
    orifice_velocity, inlet_velocity, inlet_warning = calculate_valve_velocity(scenario, valve, opening)
...
def plot_cv_curve(valve, op_points, req_cvs, theoretical_cvs, scenario_names):
    ...
    for i, (op, req_cv, theoretical_cv) in enumerate(zip(op_points, req_cvs, theoretical_cvs)):
        ...
        fig.add_trace(go.Scatter(
            ...
            showlegend=(i==0)  # Show only for first scenario
        ))
        fig.add_trace(go.Scatter(
            x=[op], 
            y=[actual_kv], 
...
def main():
    ...
    with st.sidebar:
        ...
        # UPDATED: Cv Characteristics with Kv column
            ...
            for i, scenario in enumerate(scenarios):
                ...
                with st.container():
                    ...
                    with st.expander(f"Detailed Calculations for {scenario['name']}"):
                        ...
                        col1, col2 = st.columns(2)
                        with col1:
                            # Display effective Fl/Xt values
                            if scenario["fluid_type"] == "liquid":
                            ...
                            if isinstance(fp_val, (int, float)):
                                ...
                                # Show which Cv was used for Fp calculation
                        ...
                        # Show iteration details if available - NOW INCLUDES FINAL ITERATION
