...
# ========================
# ENHANCED STEAM PROPERTY FUNCTIONS
# ========================

def calculate_steam_density(temp_c: float, press_bar: float) -> float:
    """Calculate density for steam (water vapor)"""
    try:
        T = temp_c + C_TO_K
        P = press_bar * 1e5
        
        # First get saturation temperature at this pressure
        try:
            T_sat = CP.PropsSI('T', 'P', P, 'Q', 1, 'Water')
        except:
            # If we can't get saturation temperature (e.g., supercritical), use direct calculation
            T_sat = T - 1  # Force superheated calculation
        
        if T > T_sat:  # Superheated
            # For superheated steam, get properties directly
            rho = CP.PropsSI('D', 'T', T, 'P', P, 'Water')
        else:  # Saturated or wet steam
            # Use quality=1 for saturated vapor
            rho = CP.PropsSI('D', 'T', T, 'Q', 1, 'Water')
            
        return rho
    except Exception as e:
        print(f"Steam density calculation error: {e}")
        # Fallback: ideal gas law for steam
        M_steam = 18.01528  # g/mol
        R = 8.314462  # J/(mol·K)
        T = temp_c + C_TO_K
        P = press_bar * 1e5
        rho = (P * M_steam) / (R * T) / 1000  # Convert to kg/m³
        return max(0.1, min(rho, 100.0))  # Ensure reasonable bounds

def calculate_steam_k(temp_c: float, press_bar: float) -> float:
    """Calculate specific heat ratio for steam"""
    try:
        T = temp_c + C_TO_K
        P = press_bar * 1e5
        ...
        try:
            ...
            if 'gas' in phase.lower() or 'vapor' in phase.lower() or 'supercritical' in phase.lower():
                Cp = CP.PropsSI('CPMASS', 'T', T, 'P', P, 'Water')
                Cv = CP.PropsSI('CVMASS', 'T', T, 'P', P, 'Water')
                return Cp / Cv
        except:
            pass
        
        # Fallback calculation
        # For superheated steam, try to get properties at the given state
        try:
            Cp = CP.PropsSI('CPMASS', 'T', T, 'P', P, 'Water')
            Cv = CP.PropsSI('CVMASS', 'T', T, 'P', P, 'Water')
            return Cp / Cv
        except:
            # Fallback for superheated steam
            if temp_c > 150:
                return 1.3 - 0.00005 * (temp_c - 150)
            else:
                return 1.33
    except:
        # Final fallback
        return 1.33  # Typical value for steam

def calculate_steam_sg(temp_c: float, press_bar: float) -> float:
    """Calculate specific gravity of steam relative to air at standard conditions"""
    try:
        # Density of steam at given conditions
        steam_density = calculate_steam_density(temp_c, press_bar)
        
        # Density of air at standard conditions (15°C, 1.01325 bar)
        T_std = 15 + C_TO_K
        P_std = 1.01325 * 1e5
        air_density = CP.PropsSI('D', 'T', T_std, 'P', P_std, 'Air')
        
        return steam_density / air_density
    except:
        # Approximate SG for steam
        if temp_c > 150:
            return 0.6 - 0.0001 * (temp_c - 150)  # SG decreases slightly with temperature
        else:
            return 0.6  # Steam is lighter than air

def calculate_steam_viscosity(temp_c: float, press_bar: float) -> float:
    """Calculate dynamic viscosity for steam (Pa·s)"""
    try:
        T = temp_c + C_TO_K
        P = press_bar * 1e5
        mu = CP.PropsSI('VISCOSITY', 'T', T, 'P', P, 'Water')  # Pa·s
        return mu
    except:
        # Sutherland's law approximation for steam viscosity
        T_ref = 291.15  # Reference temperature (K)
        mu_ref = 1.12e-5  # Reference viscosity (Pa·s) at T_ref
        S = 1104  # Sutherland constant for steam (K)
        T = temp_c + C_TO_K
        mu = mu_ref * (T / T_ref)**1.5 * (T_ref + S) / (T + S)
        return mu

def calculate_steam_kinematic_viscosity(temp_c: float, press_bar: float) -> float:
    """Calculate kinematic viscosity for steam (cSt)"""
    try:
        # Get dynamic viscosity
        mu = calculate_steam_viscosity(temp_c, press_bar)  # Pa·s
        
        # Get density
        rho = calculate_steam_density(temp_c, press_bar)  # kg/m³
        
        # Calculate kinematic viscosity (convert from m²/s to cSt)
        nu = (mu / rho) * 1e6  # cSt
        
        return nu
    except:
        # Approximate kinematic viscosity for steam
        if temp_c > 100:
            return 0.02 + 0.0001 * (temp_c - 100)  # Increases slightly with temperature
        else:
            return 0.02  # Typical value for steam in cSt

# ========================
# FLUID LIBRARY WITH ENHANCED STEAM PROPERTIES
# ========================
FLUID_LIBRARY = {
    "Water": {
        "type": "liquid",
        ...
        "sg_func": lambda t, p: 1.0  # Water specific gravity is 1.0
    },
    "air": {
        "type": "gas",
        "coolprop_name": "air",
        "sg_func": lambda t, p: 1.0,
        "visc_func": None,
    ...
    "Steam": {
        "type": "steam",
        "coolprop_name": "Water",
        "sg_func": lambda t, p: calculate_steam_sg(t, p),
        "visc_func": lambda t, p: calculate_steam_kinematic_viscosity(t, p),
        "k_func": lambda t, p: calculate_steam_k(t, p),
        "z_func": None,
        "pv_func": None,
        "pc_func": None,
        "rho_func": lambda t, p: calculate_steam_density(t, p)
    },
    "Superheated Steam": {
        "type": "steam",
        "coolprop_name": "Water",
    ...
    "20% Hydrogen + 80% Methane": {
        "type": "gas",
        "coolprop_name": None,
        "sg_func": lambda t, p: 0.455,  # Calculated as (0.2*0.0696 + 0.8*0.554)
        "visc_func": None,
    ...
    "Natural Gas": {
        ...
        "sg_func": lambda t, p: 0.6,  # Typical specific gravity for natural gas (range: 0.55-0.75)
        "visc_func": None,
        "k_func": lambda t, p: calculate_specific_heat_ratio("Methane", t, p),
        "z_func": lambda t, p: calculate_compressibility_factor("Methane", t, p),
        "pv_func": None,
    ...
    "JP8": {
        ...
        "sg_func": lambda t, p: 0.81,  # Typical specific gravity for JP8 (0.78-0.84)
    ...
    "JET A1": {
        ...
        "sg_func": lambda t, p: 0.80,  # Typical specific gravity for Jet A1 (0.78-0.84)
...
def calculate_kinematic_viscosity(fluid: str, temp_c: float, press_bar: float) -> float:
    ...
    if fluid.lower() == "water":
        # Water viscosity approximation (cSt)
...
def calculate_specific_heat_ratio(fluid: str, temp_c: float, press_bar: float) -> float:
    ...
    if fluid.lower() == "air":
        ...
        return calculate_steam_k(temp_c, press_bar)
...
# ========================
# TEST STEAM PROPERTIES FUNCTION
# ========================
def test_steam_properties():
    """Test CoolProp steam properties calculation"""
    test_cases = [
        (200, 10, "Superheated"),  # 200°C, 10 bar
        (300, 20, "Superheated"),  # 300°C, 20 bar
        (150, 5, "Superheated"),   # 150°C, 5 bar
        (100, 1, "Saturated"),     # 100°C, 1 bar (saturation)
    ]
    
    results = []
    for temp_c, press_bar, expected in test_cases:
        try:
            # Calculate properties
            rho = calculate_steam_density(temp_c, press_bar)
            k = calculate_steam_k(temp_c, press_bar)
            sg = calculate_steam_sg(temp_c, press_bar)
            visc = calculate_steam_kinematic_viscosity(temp_c, press_bar)
            ...
            results.append({
                "Temp (°C)": temp_c,
                "Pressure (bar)": press_bar,
                "Density (kg/m³)": f"{rho:.2f}",
                "k (Cp/Cv)": f"{k:.3f}",
                "SG (air=1)": f"{sg:.3f}",
                "Visc (cSt)": f"{visc:.4f}",
                "Phase": phase,
                "Expected": expected,
                "Status": "OK" if expected in phase else "CHECK"
            })
        except Exception as e:
            results.append({
                "Temp (°C)": temp_c,
...
# ========================
# ENHANCED CV_STEAM FUNCTION WITH IMPROVED PROPERTIES
# ========================
def cv_steam(flow: float, p1: float, p2: float, rho: float, k: float, 
    ...
    # Ensure k is reasonable for steam
    if k < 1.1 or k > 1.5:
        k = 1.33  # Force reasonable value for steam
    ...
    # Add steam property diagnostics
    steam_debug = {
        'input_rho': rho,
        'input_k': k,
        'corrected_k': k,
        ...
        'x_crit': x_crit,
        'is_choked': is_choked,
        'density_check': 'OK' if 0.1 < rho < 100 else f'Unusual density: {rho:.2f} kg/m³'
    }
...
def generate_simple_pdf_report(scenarios, valve, op_points, req_cvs, warnings, cavitation_info,
    ...
    try:
        ...
        for i, scenario in enumerate(scenarios):
            ...
            pdf.add_key_value("Theoretical Cv", f"{req_cvs[i]:.1f}")
...
def calculate_valve_velocity(scenario, valve, op_point):
    ...
    area_full = math.pi * (diameter_in * 0.0254 / 2)**2
...
def evaluate_valve_for_scenario(valve, scenario):
    ...
    if scenario["fluid_type"] == "liquid":
        if scenario.get('fluid_library') in FLUID_LIBRARY:
            ...
            if "sg_func" in fluid_data:
                scenario["sg"] = fluid_data["sg_func"](scenario["temp"], scenario["p1"])
...
def update_fluid_properties(scenario_num, fluid_library, temp, p1):
    ...
    if fluid_library != "Select Fluid Library...":
        if fluid_library in FLUID_LIBRARY:
            ...
            # Update all properties based on fluid type
            if fluid_data["type"] == "liquid":
                ...
                if fluid_data.get("sg_func"):
                    st.session_state[f"sg_{scenario_num}"] = fluid_data["sg_func"](temp, p1)
...
def scenario_input_form(scenario_num, scenario_data=None):
    ...
    with col2:
        ...
        if fluid_type in ["liquid", "gas", "steam"]:
            sg = st.number_input(
                "Specific Gravity" + (" (water=1)" if fluid_type == "liquid" else " (air=1)"),
                min_value=0.01, 
                max_value=10.0, 
                value=sg_value, 
                step=0.01,
        ...
        if fluid_type == "steam":
            ...
            # Show viscosity for steam
            if current_fluid_library != "Select Fluid Library...":
                st.caption(f"Steam viscosity: {visc_value:.3f} cSt")
    ...
    return {
        ...
        "p2": p2,
        "temp": temp,
        "sg": sg if fluid_type in ["liquid", "gas", "steam"] else scenario_data["sg"],
