...
BAR_TO_PSI = 14.5038  # Added for noise calculation
G_CONST = 9.80665
...
# ========================
# AERODYNAMIC NOISE CONSTANTS
# ========================
# Based on Hans D. Baumann's ABC method from the provided Excel sheet
AIR_MOLECULAR_MASS = 28.97  # AMU
AIR_SONIC_VELOCITY_FPS = 1126  # ft/sec
AIR_SONIC_VELOCITY_MPS = 343  # m/sec
...
# ========================
# AERODYNAMIC NOISE CALCULATION FUNCTIONS
# ========================
def calculate_aerodynamic_noise_abc(p1_bar, p2_bar, molecular_mass, sonic_velocity_mps,
                                   pipe_diameter_mm, cv_value, fl_value, fd_value,
                                   pipe_schedule_db=0, valve_style_db=0,
                                   pressure_unit="bar(a)", velocity_unit="m/sec", 
                                   pipe_size_unit="mm"):
    """
    Calculate aerodynamic noise using Hans D. Baumann's ABC method
    
    Parameters:
    -----------
    p1_bar, p2_bar: Inlet and outlet pressures in bar(a)
    molecular_mass: Molecular mass in AMU (Air = 28.97)
    sonic_velocity_mps: Sonic velocity in m/s (Air = 343 m/s)
    pipe_diameter_mm: Downstream pipe diameter in mm
    cv_value: Valve capacity Cv at operating point
    fl_value: Pressure recovery factor FL at operating point
    fd_value: Valve style modifier Fd at operating point
    pipe_schedule_db: Pipe schedule correction (40=0, 80=4, 160=8)
    valve_style_db: Valve style correction (globe=3, others=0)
    
    Returns:
    --------
    Dictionary containing noise level and intermediate calculations
    """
    
    # Convert inputs to calculation units (psia, ft/sec, inch)
    if pressure_unit == "bar(a)":
        p1_psia = p1_bar * BAR_TO_PSI
        p2_psia = p2_bar * BAR_TO_PSI
    elif pressure_unit == "psia":
        p1_psia = p1_bar
        p2_psia = p2_bar
    elif pressure_unit == "kPa(a)":
        p1_psia = p1_bar * 0.145  # kPa to psia
        p2_psia = p2_bar * 0.145
    else:
        raise ValueError(f"Unsupported pressure unit: {pressure_unit}")
    
    if velocity_unit == "m/sec":
        sonic_velocity_fps = sonic_velocity_mps * 3.2808
    elif velocity_unit == "ft/sec":
        sonic_velocity_fps = sonic_velocity_mps
    else:
        raise ValueError(f"Unsupported velocity unit: {velocity_unit}")
    
    if pipe_size_unit == "mm":
        pipe_diameter_inch = pipe_diameter_mm * 0.03937
    elif pipe_size_unit == "inch":
        pipe_diameter_inch = pipe_diameter_mm
    else:
        raise ValueError(f"Unsupported pipe size unit: {pipe_size_unit}")
    
    # Calculate pressure ratio X
    if p1_psia <= 0:
        return {"error": "Inlet pressure must be positive", "spl_dba": 0}
    
    X = (p1_psia - p2_psia) / p1_psia
    
    # Calculate DFL (correction for FL)
    if fl_value < 0.7:
        DFL = 30 * math.log10(0.7 / fl_value)
    else:
        DFL = 30 * math.log10(fl_value / 0.7)
    
    # Calculate component A
    A = 25 * math.log10(pipe_diameter_inch) - 10 + 20 * math.log10(cv_value / (pipe_diameter_inch**2))
    
    # Calculate component B
    B = 35 * math.log10(p1_psia) - 35
    
    # Calculate component C
    C = 27 + 30 + 30 * math.log10(X) + 20 * math.log10(fd_value)
    
    # Calculate component G (correction for gases other than air)
    G = 20 * math.log10(sonic_velocity_fps / AIR_SONIC_VELOCITY_FPS) + 15 * math.log10(molecular_mass / AIR_MOLECULAR_MASS)
    
    # Calculate final sound pressure level LpAe1m
    LpAe1m = A + B + C + DFL + G - valve_style_db - pipe_schedule_db
    ...
    results = {
        ...
        "intermediate": {
            "p1_psia": p1_psia,
            "p2_psia": p2_psia,
            "sonic_velocity_fps": sonic_velocity_fps,
            "pipe_diameter_inch": pipe_diameter_inch,
            "pressure_ratio_X": X
        },
        "formula_used": "LpAe1m = A + B + C + DFL + G - rw - ps"
    }
    
    return results

def get_sonic_velocity_gas(temperature_c, molecular_mass, specific_heat_ratio=1.4):
    """
    Calculate sonic velocity of a gas using formula from Excel sheet:
    c = sqrt(gamma * R * T / M)
    where:
    gamma = specific heat ratio
    R = universal gas constant = 8314 J/(kmol·K)
    T = temperature in Kelvin
    M = molecular mass in kg/kmol (same as AMU)
    
    Returns sonic velocity in m/s
    """
    # Convert temperature to Kelvin
    T_k = temperature_c + C_TO_K
    
    # Calculate sonic velocity
    sonic_velocity_mps = math.sqrt(specific_heat_ratio * 8314 * T_k / molecular_mass)
    
    return sonic_velocity_mps

def aerodynamic_noise_tab(valve, scenarios, valve_results):
    """
    Display aerodynamic noise calculation tab
    """
    st.header("🔊 Aerodynamic Noise Prediction")
    st.markdown("""
    **ABC Method for predicting control valve aerodynamic noise**  
    *Calculation method by Hans D. Baumann*  
    *Excel worksheet by Jon F. Monsen*
    
    This method estimates the sound pressure level (SPL) at 1 meter from the valve and downstream pipe.
    """)
    
    # Create tabs for different scenarios
    if scenarios:
        scenario_tabs = st.tabs([f"{s['name']}" for s in scenarios if s['fluid_type'] == 'gas'])
        
        for idx, (tab, scenario) in enumerate(zip(scenario_tabs, [s for s in scenarios if s['fluid_type'] == 'gas'])):
            with tab:
                ...
                with col1:
                    ...
                    # Get operating point from valve results
                    op_point = valve_results[idx]["op_point"] if idx < len(valve_results) else 50
                    ...
                    # Molecular mass
                    molecular_mass = st.number_input(
                        "Molecular Mass (AMU, Air = 28.97)",
                        min_value=1.0,
                        max_value=200.0,
                        value=scenario.get("sg", 1.0) * AIR_MOLECULAR_MASS,
                        step=0.01,
                        key=f"noise_mass_{idx}"
                    )
                    
                    # Sonic velocity
                    col_vel1, col_vel2 = st.columns([2, 1])
                    with col_vel1:
                        sonic_velocity = st.number_input(
                            "Sonic Velocity",
                            min_value=1.0,
                            max_value=2000.0,
                            value=get_sonic_velocity_gas(scenario["temp"], molecular_mass, scenario.get("k", 1.4)),
                            step=1.0,
                            key=f"noise_velocity_{idx}"
                        )
                    with col_vel2:
                        velocity_unit = st.selectbox(
                            ...
                            ["m/sec", "ft/sec"],
                            index=0,
                            key=f"noise_velocity_unit_{idx}"
                        )
                    
                    # Calculate button for sonic velocity
                    if st.button("Calculate Sonic Velocity from Temperature", key=f"calc_velocity_{idx}"):
                        temp = scenario["temp"]
                        k = scenario.get("k", 1.4)
                        calculated_velocity = get_sonic_velocity_gas(temp, molecular_mass, k)
                        st.session_state[f"noise_velocity_{idx}"] = calculated_velocity
                        st.success(f"Calculated sonic velocity: {calculated_velocity:.1f} m/s")
                
                with col2:
                    st.subheader("Valve/Pipe Parameters")
                    
                    # Downstream pipe diameter
                    col_pipe1, col_pipe2 = st.columns([2, 1])
                    with col_pipe1:
                        pipe_diameter = st.number_input(
                            "Downstream Pipe Diameter",
                            min_value=1.0,
                            max_value=2000.0,
                            value=scenario.get("pipe_d", valve.diameter) * 25.4,  # Convert inch to mm
                            step=1.0,
                            key=f"noise_pipe_dia_{idx}"
                        )
                    with col_pipe2:
                        pipe_unit = st.selectbox(
                            ...
                            ["mm", "inch"],
                            index=0,
                            key=f"noise_pipe_unit_{idx}"
                        )
                    
                    # Valve capacity Cv at operating point
                    cv_value = st.number_input(
                        "Valve Capacity Cv at Operating Point",
                        min_value=0.1,
                        max_value=10000.0,
                        value=valve.get_cv_at_opening(op_point),
                        step=1.0,
                        key=f"noise_cv_{idx}"
                    )
                    
                    # Pressure recovery factor FL at operating point
                    fl_value = st.number_input(
                        "Pressure Recovery Factor FL at Operating Point",
                        min_value=0.1,
                        max_value=1.5,
                        value=valve.get_fl_at_opening(op_point),
                        step=0.01,
                        key=f"noise_fl_{idx}"
                    )
                    
                    # Valve style modifier Fd at operating point
                    fd_value = st.number_input(
                        "Valve Style Modifier Fd at Operating Point",
                        min_value=0.1,
                        max_value=2.0,
                        value=valve.fd,
                ...
                with col_mod1:
                    pipe_schedule = st.selectbox(
                        "Downstream Pipe Schedule",
                        options=["Schedule 40 (0 dB)", "Schedule 80 (4 dB)", "Schedule 160 (8 dB)"],
                        index=0,
                        key=f"noise_pipe_sched_{idx}"
                    )
                    
                    # Extract dB value from selection
                    pipe_schedule_db = 0
                    if "80" in pipe_schedule:
                        pipe_schedule_db = 4
                    elif "160" in pipe_schedule:
                        pipe_schedule_db = 8
                
                with col_mod2:
                    valve_style = st.selectbox(
                        "Valve Style",
                        options=["Globe (3 dB)", "Other (0 dB)"],
                        index=0 if valve.valve_type == 3 else 1,
                        key=f"noise_valve_style_{idx}"
                    )
                    
                    # Extract dB value from selection
                    valve_style_db = 3 if "Globe" in valve_style else 0
                ...
                with col_unit1:
                    pressure_unit = st.selectbox(
                        ...
                        ["psia", "bar(a)", "kPa(a)"],
                        index=1,
                        key=f"noise_pressure_unit_{idx}"
                    )
                
                with col_unit2:
                    # Velocity unit already selected above
                    pass
                
                with col_unit3:
                    # Pipe size unit already selected above
                    pass
                
                # Calculate noise button
                if st.button("Calculate Aerodynamic Noise", key=f"calc_noise_{idx}"):
                    with st.spinner("Calculating noise level..."):
                        try:
                            # Calculate aerodynamic noise
                            noise_results = calculate_aerodynamic_noise_abc(
                                p1_bar=p1,
                                p2_bar=p2,
                                molecular_mass=molecular_mass,
                                sonic_velocity_mps=sonic_velocity,
                                pipe_diameter_mm=pipe_diameter,
                                cv_value=cv_value,
                                fl_value=fl_value,
                                fd_value=fd_value,
                                pipe_schedule_db=pipe_schedule_db,
                                valve_style_db=valve_style_db,
                                pressure_unit=pressure_unit,
                                velocity_unit=velocity_unit,
                                pipe_size_unit=pipe_unit
                            )
                            
                            if "error" in noise_results:
                                st.error(f"Calculation error: {noise_results['error']}")
                            else:
                                # Display results
                                st.success(f"### Estimated Sound Pressure Level (SPL): **{noise_results['spl_dba']:.1f} dB(A)**")
                                
                                # Create visualization
                                fig = go.Figure()
                                
                                # Add bars for each component
                                components_data = noise_results["components"]
                                component_names = list(components_data.keys())
                                component_values = list(components_data.values())
                                ...
                                fig.add_trace(go.Bar(
                                    ...
                                    textposition='auto',
                                    marker_color=colors[:len(component_names)],
                                    name="Components"
                                ))
                                
                                # Add total line
                                fig.add_hline(y=noise_results["spl_dba"], 
                                            line_dash="dash", 
                                            line_color="red",
                                            annotation_text=f"Total SPL: {noise_results['spl_dba']:.1f} dB(A)",
                                            annotation_position="top right")
                                
                                fig.update_layout(
                                    title="Noise Level Components Breakdown",
                                    xaxis_title="Component",
                                    yaxis_title="dB(A) Contribution",
                                    showlegend=False,
                                    height=400,
                                ...
                                with st.expander("Detailed Calculation Breakdown"):
                                    ...
                                    data = []
                                    total = 0
                                    for comp, value in components_data.items():
                                        total += value
                                        data.append({
                                            "Component": comp,
                                            "Value (dB)": f"{value:.2f}",
                                            "Cumulative Total": f"{total:.2f}"
                                        })
                                    ...
                                    intermediate = noise_results["intermediate"]
                                    col_int1, col_int2 = st.columns(2)
                                    
                                    with col_int1:
                                        st.metric("Inlet Pressure", f"{intermediate['p1_psia']:.2f} psia")
                                        st.metric("Outlet Pressure", f"{intermediate['p2_psia']:.2f} psia")
                                        st.metric("Pressure Ratio X", f"{intermediate['pressure_ratio_X']:.4f}")
                                    
                                    with col_int2:
                                        st.metric("Sonic Velocity", f"{intermediate['sonic_velocity_fps']:.1f} ft/sec")
                                        st.metric("Pipe Diameter", f"{intermediate['pipe_diameter_inch']:.2f} inch")
                                    
                                    st.markdown(f"**Formula used:** {noise_results['formula_used']}")
                                
                                # Show noise level interpretation
                                with st.expander("Noise Level Interpretation"):
                                    spl = noise_results["spl_dba"]
                                    
                                    if spl < 70:
                                        st.success("**Low Noise (< 70 dB(A))**: Generally acceptable for most applications.")
                                        st.info("""
                                        **Typical environments:**  
                                        • Normal office environments  
                                        • Residential areas  
                                        • Most indoor industrial settings
                                        """)
                                    elif spl < 85:
                                        st.warning("**Moderate Noise (70-85 dB(A))**: May require hearing protection for prolonged exposure.")
                                        st.info("""
                                        **Typical environments:**  
                                        • Factory floors  
                                        • Outdoor industrial areas  
                                        • May require administrative controls
                                        """)
                                    elif spl < 100:
                                        st.error("**High Noise (85-100 dB(A))**: Requires hearing protection and engineering controls.")
                                        st.info("""
                                        **Typical environments:**  
                                        • Loud industrial equipment  
                                        • Requires PPE and noise enclosures  
                                        • Consider noise abatement measures
                                        """)
                                    else:
                                        st.error("**Very High Noise (> 100 dB(A))**: Serious risk, requires immediate mitigation.")
                                        st.info("""
                                        **Immediate actions required:**  
                                        • Engineering controls mandatory  
                                        • Double hearing protection  
                                        • Limit exposure time  
                                        • Consider alternative valve types or locations
                                        """)
                                    
                                    # Add noise reduction suggestions
                                    st.markdown("### Noise Reduction Suggestions")
                                    suggestions = []
                                    
                                    if fl_value < 0.7:
                                        suggestions.append("Consider a valve with higher FL factor to reduce noise")
                                    if pipe_diameter < 50:  # mm
                                        suggestions.append("Increasing pipe diameter can reduce noise")
                                    if fd_value > 1.0:
                                        suggestions.append("Consider a valve with lower Fd (different trim design)")
                                    ...
                                    if suggestions:
                                        ...
                                        st.info("Current configuration is relatively optimized for noise control.")
                                
                                # Save results to session state
                                if 'noise_results' not in st.session_state:
                                    st.session_state.noise_results = {}
                                st.session_state.noise_results[idx] = noise_results
                                
                        except Exception as e:
                            st.error(f"Error calculating aerodynamic noise: {str(e)}")
                            st.error(traceback.format_exc())
    else:
        st.info("No gas flow scenarios found. Aerodynamic noise calculation requires gas flow scenarios.")
        
        # Allow manual input even without scenarios
        st.subheader("Manual Noise Calculation")
        
        col1, col2 = st.columns(2)
        
        with col1:
            st.subheader("Process Parameters")
            p1 = st.number_input("Inlet Pressure (bar a)", min_value=0.1, max_value=1000.0, value=10.0, step=0.1)
            p2 = st.number_input("Outlet Pressure (bar a)", min_value=0.0, max_value=1000.0, value=6.0, step=0.1)
            molecular_mass = st.number_input("Molecular Mass (AMU)", min_value=1.0, max_value=200.0, value=28.97, step=0.01)
            sonic_velocity = st.number_input("Sonic Velocity (m/s)", min_value=1.0, max_value=2000.0, value=343.0, step=1.0)
        
        with col2:
            st.subheader("Valve/Pipe Parameters")
            pipe_diameter = st.number_input("Downstream Pipe Diameter (mm)", min_value=1.0, max_value=2000.0, value=50.0, step=1.0)
            cv_value = st.number_input("Valve Capacity Cv", min_value=0.1, max_value=10000.0, value=50.0, step=1.0)
            fl_value = st.number_input("Pressure Recovery Factor FL", min_value=0.1, max_value=1.5, value=0.9, step=0.01)
            fd_value = st.number_input("Valve Style Modifier Fd", min_value=0.1, max_value=2.0, value=0.25, step=0.01)
        
        # Calculate button
        if st.button("Calculate Aerodynamic Noise"):
            noise_results = calculate_aerodynamic_noise_abc(
                p1_bar=p1,
                p2_bar=p2,
                molecular_mass=molecular_mass,
                ...
                fd_value=fd_value,
                pipe_schedule_db=0,
                valve_style_db=3 if valve.valve_type == 3 else 0
            )
            
            if "error" not in noise_results:
                st.success(f"### Estimated Sound Pressure Level: **{noise_results['spl_dba']:.1f} dB(A)**")
...
def main():
    ...
    # Create tabs including the new Aerodynamic Noise tab
    tab1, tab2, tab3, tab_results, tab_noise = st.tabs(["Scenario 1", "Scenario 2", "Scenario 3", "Results", "Aerodynamic Noise"])
    ...
    with tab_noise:
        # Call the aerodynamic noise tab function
        if st.session_state.results:
            valve = st.session_state.results["selected_valve"]
            valve_results = st.session_state.results["selected_valve_results"]
            aerodynamic_noise_tab(valve, scenarios, valve_results)
        else:
            st.info("Please run the valve sizing calculation first to enable aerodynamic noise prediction.")
            # Still allow manual calculation
            aerodynamic_noise_tab(selected_valve, [], [])
