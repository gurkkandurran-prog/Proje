...
FLUID_LIBRARY = {
    ...
    "Steam": {
        "type": "steam",
        "coolprop_name": "Water",
        "sg": None,  # Steam doesn't have specific gravity
        "visc_func": None,
...
def reset_fluid_properties(scenario_data, fluid_library, temp, p1):
    """Reset all fluid properties based on fluid library selection"""
    if fluid_library != "Select Fluid Library...":
        ...
        # Reset all properties to defaults first
        if fluid_data["type"] == "liquid":
            ...
            scenario_data.update({
                "sg": 1.0,  # Steam doesn't use SG but reset anyway
                "visc": 1.0,
                "pv": 0.0,
                "pc": 0.0,
                "k": 1.33,
        ...
        # Now update with library values
        if "sg" in fluid_data and fluid_data["sg"] is not None:
...
def update_fluid_properties(scenario_data, temp, p1):
    """Update fluid properties when temperature or pressure changes"""
    fluid_library = scenario_data.get("fluid_library")
    
    if fluid_library and fluid_library != "Select Fluid Library...":
        fluid_data = FLUID_LIBRARY[fluid_library]
        
        # Update calculated properties based on current conditions
        if fluid_data.get("visc_func") and fluid_data["type"] == "liquid":
            scenario_data["visc"] = fluid_data["visc_func"](temp, p1)
        if fluid_data.get("pv_func") and fluid_data["type"] == "liquid":
            scenario_data["pv"] = fluid_data["pv_func"](temp, p1)
...
def scenario_input_form(scenario_num, scenario_data=None):
    ...
    with col1:
        # Store previous fluid library to detect changes
        previous_fluid_library = scenario_data.get("fluid_library", "Select Fluid Library...")
    ...
    with col2:
        # Show fluid type based on selection
        if fluid_library != "Select Fluid Library...":
            fluid_data = FLUID_LIBRARY[fluid_library]
    ...
    with col2:
        # Check if fluid library changed and reset properties if needed
        if fluid_library != previous_fluid_library:
            scenario_data = reset_fluid_properties(scenario_data, fluid_library, temp, p1)
        else:
            # Update properties based on current temperature and pressure
            scenario_data = update_fluid_properties(scenario_data, temp, p1)
        
        # Show appropriate input fields based on fluid type
