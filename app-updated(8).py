...
def scenario_input_form(scenario_num, scenario_data=None):
    ...
    with col2:
        ...
        if current_fluid_library != "Select Fluid Library...":
            fluid_data = FLUID_LIBRARY[current_fluid_library]
            recalculated_props["fluid_type"] = fluid_data["type"]
            
            # Recalculate all fluid properties based on current temp and pressure
            if fluid_data.get("visc_func") and fluid_data["type"] == "liquid":
                recalculated_props["visc"] = fluid_data["visc_func"](current_temp, current_p1)
            if fluid_data.get("k_func") and fluid_data["type"] in ["gas", "steam"]:
                recalculated_props["k"] = fluid_data["k_func"](current_temp, current_p1)
            if fluid_data.get("pv_func") and fluid_data["type"] == "liquid":
                recalculated_props["pv"] = fluid_data["pv_func"](current_temp, current_p1)
            if fluid_data.get("pc_func") and fluid_data["type"] == "liquid":
                recalculated_props["pc"] = fluid_data["pc_func"]()
            if fluid_data.get("rho_func") and fluid_data["type"] == "steam":
                recalculated_props["rho"] = fluid_data["rho_func"](current_temp, current_p1)
            if fluid_data.get("z_func") and fluid_data["type"] == "gas":
                recalculated_props["z"] = fluid_data["z_func"](current_temp, current_p1)
            if fluid_data.get("sg") is not None:
                recalculated_props["sg"] = fluid_data["sg"]
        
        # Use recalculated properties if available, otherwise use previous values
        sg_value = recalculated_props.get("sg", scenario_data["sg"])
        visc_value = recalculated_props.get("visc", scenario_data["visc"])
        pv_value = recalculated_props.get("pv", scenario_data["pv"])
        pc_value = recalculated_props.get("pc", scenario_data["pc"])
        k_value = recalculated_props.get("k", scenario_data["k"])
        z_value = recalculated_props.get("z", scenario_data["z"])
        rho_value = recalculated_props.get("rho", scenario_data["rho"])
