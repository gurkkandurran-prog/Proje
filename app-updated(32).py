...
# UPDATED: FLP calculation using Fl at operating point
def calculate_flp(valve_d_inch: float, pipe_d_inch: float, cv_100: float, fl_at_op: float) -> float:
    if abs(pipe_d_inch - valve_d_inch) < 0.01:
        return fl_at_op  # No reducer, FLP is same as FL
    d_ratio = valve_d_inch / pipe_d_inch
    K1 = 0.5 * (1 - d_ratio**2)**2
    KB1 = 1 - d_ratio**4
    Ki = K1 + KB1
    term = (Ki / CONSTANTS["N2"]["inch"]) * (cv_100 / valve_d_inch**2)**2 + 1/fl_at_op**2
    FLP = 1 / math.sqrt(term)
    return FLP

# UPDATED: XTP calculation using Xt at operating point
def calculate_x_tp(valve_d_inch: float, pipe_d_inch: float, Fp: float, xt_at_op: float, cv_100: float) -> float:
    if abs(pipe_d_inch - valve_d_inch) < 0.01:
        return xt_at_op  # No reducer, xTP is same as xT
    xT = xt_at_op
    d_ratio = valve_d_inch / pipe_d_inch
    K1 = 0.5 * (1 - d_ratio**2)**2
    KB1 = 1 - d_ratio**4
...
# ========================
# ITERATION AND CALCULATION REPORT
# ========================
def generate_iteration_report(scenario, valve, result):
    """
    Generate a detailed iteration and calculation report showing formulas and steps
    """
    report = []
    
    # Header
    report.append(f"# ITERATION AND CALCULATION REPORT")
    report.append(f"**Scenario:** {scenario['name']}")
    report.append(f"**Valve:** {get_valve_display_name(valve)}")
    report.append(f"**Fluid Type:** {scenario['fluid_type']}")
    report.append(f"**Date:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    report.append("---")
    
    # Basic Parameters
    report.append("## 1. INPUT PARAMETERS")
    report.append("### 1.1 Flow Conditions")
    report.append(f"- Flow Rate: {scenario['flow_display']:.2f} {scenario['flow_unit']}")
    report.append(f"- Inlet Pressure (P1): {scenario['p1']:.2f} bar")
    report.append(f"- Outlet Pressure (P2): {scenario['p2']:.2f} bar")
    report.append(f"- Pressure Drop (ΔP): {scenario['p1'] - scenario['p2']:.2f} bar")
    report.append(f"- Temperature: {scenario['temp']:.1f} °C")
    
    if scenario['fluid_type'] == 'liquid':
        report.append("### 1.2 Fluid Properties (Liquid)")
        report.append(f"- Specific Gravity (SG): {scenario['sg']:.4f}")
        report.append(f"- Viscosity (ν): {scenario['visc']:.2f} cSt")
        report.append(f"- Vapor Pressure (Pv): {scenario['pv']:.4f} bar")
        report.append(f"- Critical Pressure (Pc): {scenario['pc']:.2f} bar")
    elif scenario['fluid_type'] == 'gas':
        report.append("### 1.2 Fluid Properties (Gas)")
        report.append(f"- Specific Gravity (SG): {scenario['sg']:.4f}")
        report.append(f"- Specific Heat Ratio (k): {scenario['k']:.3f}")
        report.append(f"- Compressibility Factor (Z): {scenario['z']:.3f}")
    else:  # steam
        report.append("### 1.2 Fluid Properties (Steam)")
        report.append(f"- Density (ρ): {scenario['rho']:.2f} kg/m³")
        report.append(f"- Specific Heat Ratio (k): {scenario['k']:.3f}")
    
    report.append("### 1.3 Valve Parameters")
    report.append(f"- Valve Size: {valve.size}\"")
    report.append(f"- Valve Type: {'Globe' if valve.valve_type == 3 else 'Axial'}")
    report.append(f"- Rating Class: {valve.rating_class}")
    report.append(f"- Fd (Style Modifier): {valve.fd:.2f}")
    report.append(f"- Pipe Diameter: {scenario['pipe_d']}\"")
    
    report.append("---")
    
    # Calculation Formulas
    report.append("## 2. CALCULATION FORMULAS")
    
    if scenario['fluid_type'] == 'liquid':
        report.append("### 2.1 Liquid Flow Formulas (ISA/IEC 60534-2-1)")
        report.append("#### Basic Cv Formula:")
        report.append(r"$$C_v = \frac{Q}{N_1} \sqrt{\frac{SG}{\Delta P}}$$")
        report.append(f"- Where N₁ = {CONSTANTS['N1']['m³/h, bar']:.3f} for m³/h and bar")
        report.append(r"#### Choked Flow Condition:")
        report.append(r"$$\Delta P_{max} = F_L^2 (P_1 - F_F \cdot P_v)$$")
        report.append(r"#### Critical Pressure Ratio Factor (F_F):")
        report.append(r"$$F_F = 0.96 - 0.28 \sqrt{\frac{P_v}{P_c}}$$")
        report.append(r"#### Reynolds Number for Viscosity Correction:")
        report.append(r"$$Re = \frac{N_4 \cdot F_d \cdot Q}{\nu \cdot \sqrt{F_L} \cdot \sqrt{C_v}} \left[ \frac{(F_L^2 \cdot C_v^2)}{(N_2 \cdot D^4)} + 1 \right]$$")
        report.append(f"- Where N₂ = {CONSTANTS['N2']['mm']:.5f}, N₄ = {CONSTANTS['N4']['mm']:,}")
        
    elif scenario['fluid_type'] == 'gas':
        report.append("### 2.2 Gas Flow Formulas (ISA/IEC 60534-2-1)")
        report.append(r"#### Pressure Drop Ratio:")
        report.append(r"$$x = \frac{\Delta P}{P_1}$$")
        report.append(r"#### Critical Pressure Drop Ratio:")
        report.append(r"$$x_{crit} = F_k \cdot x_T$$")
        report.append(r"#### Expansion Factor (Y):")
        report.append(r"$$Y = 1 - \frac{x}{3 \cdot F_k \cdot x_T} \quad (\text{for } x < x_{crit})$$")
        report.append(r"$$Y = 0.667 \quad (\text{for } x \geq x_{crit})$$")
        report.append(r"#### Cv Formula for Gases:")
        report.append(r"$$C_v = \frac{Q}{N_7 \cdot F_p \cdot P_1 \cdot Y} \sqrt{\frac{SG \cdot T \cdot Z}{x}}$$")
        report.append(f"- Where N₇ = {CONSTANTS['N7']['m³/h, bar, K (standard)']:.1f} for m³/h, bar, K")
        
    else:  # steam
        report.append("### 2.3 Steam Flow Formulas (ISA/IEC 60534-2-1)")
        report.append(r"#### Cv Formula for Steam:")
        report.append(r"$$C_v = \frac{W}{N_6 \cdot Y \cdot F_p} \cdot \frac{1}{\sqrt{x \cdot P_1 \cdot \rho}}$$")
        report.append(f"- Where N₆ = {CONSTANTS['N6']['kg/h, bar, kg/m³']:.2f} for kg/h, bar, kg/m³")
    
    report.append("---")
    
    # Iteration Steps
    if 'iteration_history' in result:
        report.append("## 3. ITERATION HISTORY")
        
        for i, iteration in enumerate(result['iteration_history']):
            report.append(f"### Iteration {i+1}")
            report.append(f"**Operating Point:** {iteration['open_percent']:.1f}%")
            
            if scenario['fluid_type'] == 'liquid':
                report.append(f"- Fl at {iteration['open_percent']:.1f}%: {iteration['fl_at_op']:.4f}")
                report.append(f"- Fp (Piping Factor): {iteration['fp']:.4f}")
                report.append(f"- Required Cv: {iteration['cv_req']:.2f}")
                report.append(f"- Valve Cv at {iteration['open_percent']:.1f}%: {iteration['valve_cv']:.2f}")
                
                if 'reynolds' in iteration:
                    report.append(f"- Reynolds Number: {iteration['reynolds']:.0f}")
                    report.append(f"- Fr (Viscosity Correction): {iteration.get('fr', 1.0):.4f}")
                
                if 'dp_max' in iteration:
                    report.append(f"- ΔP_max (Choked Flow Limit): {iteration['dp_max']:.2f} bar")
                
                if 'is_choked' in iteration:
                    report.append(f"- Choked Flow: {'Yes' if iteration['is_choked'] else 'No'}")
                    
            elif scenario['fluid_type'] == 'gas':
                report.append(f"- Xt at {iteration['open_percent']:.1f}%: {iteration.get('xt_at_op', 0):.4f}")
                report.append(f"- Pressure Drop Ratio (x): {iteration.get('x_actual', 0):.4f}")
                report.append(f"- Critical Pressure Drop Ratio (x_crit): {iteration.get('x_crit', 0):.4f}")
                report.append(f"- Expansion Factor (Y): {iteration.get('expansion_factor', 0):.4f}")
                report.append(f"- Required Cv: {iteration['cv_req']:.2f}")
                report.append(f"- Valve Cv at {iteration['open_percent']:.1f}%: {iteration['valve_cv']:.2f}")
    ...
    # Final Results
    report.append("## 4. FINAL RESULTS")
    report.append(f"**Required Cv:** {result['req_cv']:.2f}")
    report.append(f"**Theoretical Cv:** {result['theoretical_cv']:.2f}")
    report.append(f"**Valve Cv at Operating Point:** {valve.get_cv_at_opening(result['op_point']):.2f}")
    report.append(f"**Operating Point:** {result['op_point']:.1f}%")
    report.append(f"**Capacity Margin:** {result['margin']:.1f}%")
    
    report.append("### 4.1 Velocity Analysis")
    report.append(f"- Orifice Velocity: {result.get('orifice_velocity', 0):.2f} m/s")
    report.append(f"- Inlet Velocity: {result.get('inlet_velocity', 0):.2f} m/s")
    if "High velocity" in result.get('warning', ''):
        report.append(f"**⚠️ WARNING:** {result['warning']}")
    
    report.append("### 4.2 Cavitation/Choked Flow Analysis")
    report.append(f"- Status: {result['cavitation_info']}")
    
    if scenario['fluid_type'] == 'liquid' and 'details' in result:
        details = result['details']
        if 'sigma' in details:
            report.append(f"- Sigma (σ): {details['sigma']:.2f}")
        if 'km' in details:
            report.append(f"- Km: {details['km']:.3f}")
    ...
    if result['status'] == 'green':
        report.append("✅ **OPTIMAL PERFORMANCE**")
        report.append("- Valve operates in optimal range (20-80% opening)")
        report.append("- No cavitation or choked flow issues")
        report.append("- Velocity within acceptable limits")
    elif result['status'] == 'yellow':
        report.append("⚠️ **ACCEPTABLE WITH WARNINGS**")
        report.append("- Valve operates outside optimal range or has moderate issues")
        report.append("- May require monitoring or adjustments")
    elif result['status'] == 'orange':
        report.append("⚠️ **SEVERE CAVITATION RISK**")
        report.append("- Significant cavitation potential")
        report.append("- Consider valve modifications or operating changes")
    elif result['status'] == 'red':
        report.append("❌ **UNACCEPTABLE PERFORMANCE**")
        report.append("- Choked flow detected")
        report.append("- Valve may be undersized")
        report.append("- Immediate action required")
    
    if "Insufficient" in result.get('warning', ''):
        report.append("❌ **INSUFFICIENT CAPACITY**")
        report.append("- Valve cannot handle the required flow")
        report.append("- Consider larger valve size")
    ...
    if result['op_point'] < 20:
        report.append("- **Opening too low (<20%):** Consider smaller valve for better control")
    elif result['op_point'] > 80:
        report.append("- **Opening too high (>80%):** Consider larger valve for reserve capacity")
    
    if "High velocity" in result.get('warning', ''):
        report.append("- **High velocity detected:** Consider larger pipe size or different valve trim")
    
    if "Severe" in result['cavitation_info'] or "Choked" in result['cavitation_info']:
        report.append("- **Cavitation/choked flow risk:** Consider cavitation-resistant trim or reduce pressure drop")
    
    if result['margin'] < 0:
        report.append(f"- **Negative margin ({result['margin']:.1f}%):** Valve is undersized for the application")
    elif result['margin'] > 30:
        report.append(f"- **Large margin ({result['margin']:.1f}%):** Valve may be oversized, consider smaller size")
    
    report.append("---")
    report.append("*Report generated by VASTAS Valve Sizing Software*")
    report.append("*Based on ISA-75.01.01 / IEC 60534-2-1 standards*")
    
    return "\n\n".join(report)

# ========================
# MODIFIED EVALUATE_VALVE_FOR_SCENARIO WITH ITERATION TRACKING
# ========================
def evaluate_valve_for_scenario_with_tracking(valve, scenario):
    """
    Evaluate valve for scenario with detailed iteration tracking
    """
    ...
    prev_op_point = 0
    open_percent = 50  # Start with 50% opening for all fluid types
    cv_req = 0
    details = {}
    ...
    for iteration in range(max_iterations):
        ...
        iteration_record = {
            ...
            'fp': fp,
            'valve_cv': valve.get_cv_at_opening(open_percent)
        }
        ...
        if scenario["fluid_type"] == "liquid":
            ...
            # Get Fl at current operating point
            fl_at_op = valve.get_fl_at_opening(open_percent)
            
            # Calculate FLP with Fl at operating point
            if abs(pipe_d - valve_d) > 0.01:
                fl_at_op = calculate_flp(valve_d, pipe_d, cv_100, fl_at_op)
            ...
            iteration_record.update({
                'fl_at_op': fl_at_op,
                'cv_req': cv_req,
                'reynolds': details.get('reynolds', 0),
                'fr': details.get('fr', 1.0),
                'dp_max': details.get('dp_max', 0),
                'is_choked': details.get('is_choked', False)
            })
            ...
            # Calculate xTP with Xt at operating point
            if abs(pipe_d - valve_d) > 0.01:
                xt_at_op = calculate_x_tp(valve_d, pipe_d, fp, xt_at_op, cv_100)
            ...
            iteration_record.update({
                'xt_at_op': xt_at_op,
                'cv_req': cv_req,
                'x_actual': details.get('x_actual', 0),
                'x_crit': details.get('x_crit', 0),
                'expansion_factor': details.get('expansion_factor', 0)
            })
        ...
        if iteration > 5 and abs(new_open_percent - prev_op_point) > 5:
            open_percent = (new_open_percent + prev_op_point) / 2
        else:
            prev_op_point = open_percent
            open_percent = new_open_percent
    
    # Final operating point determination
    if cv_req > 0:
        open_percent = 10
    ...
    iteration_history.append(final_record)
    ...
    if open_percent >= 100 and valve.get_cv_at_opening(open_percent) < cv_req:
...
def evaluate_valve_for_scenario(valve, scenario):
    ...
    for iteration in range(max_iterations):
        ...
        print(f"Iteration {iteration}: Open% = {open_percent}, Req Cv = {cv_req:.1f}, Valve Cv = {valve.get_cv_at_opening(open_percent):.1f}")
...
def find_recommended_valve(scenarios):
    ...
    for valve in VALVE_DATABASE:
        ...
        for scenario in scenarios:
            result = evaluate_valve_for_scenario_with_tracking(valve, scenario)
...
def main():
    ...
    if 'show_iteration_report' not in st.session_state:
        st.session_state.show_iteration_report = False
    if 'iteration_report_data' not in st.session_state:
        st.session_state.iteration_report_data = None
    ...
    if calculate_btn:
        ...
        try:
            ...
            for scenario in scenarios:
                result = evaluate_valve_for_scenario_with_tracking(selected_valve, scenario)
    ...
    with tab_results:
        if st.session_state.results:
            ...
            # Add Iteration Report Button
            col1, col2, col3 = st.columns([2, 1, 1])
            with col2:
                if st.button("📊 Show Iteration Report", use_container_width=True):
                    st.session_state.show_iteration_report = True
                    st.session_state.show_3d_viewer = False
                    st.session_state.show_simulation = False
            
            if st.session_state.show_iteration_report:
                st.markdown("---")
                st.subheader("📊 ITERATION AND CALCULATION REPORT")
                
                # Create tabs for each scenario
                scenario_tabs = st.tabs([f"Scenario {i+1}" for i in range(len(scenarios))])
                
                for i, (scenario, result) in enumerate(zip(scenarios, selected_valve_results)):
                    with scenario_tabs[i]:
                        report = generate_iteration_report(scenario, selected_valve, result)
                        
                        # Display report in expandable sections
                        with st.expander("📋 Full Report", expanded=True):
                            st.markdown(report)
                        
                        # Display iteration history in a table
                        if 'iteration_history' in result and result['iteration_history']:
                            ...
                            for iteration in result['iteration_history']:
                                if iteration['iteration'] == 'Final':
                                    continue  # Skip final record for the table
                                
                                row = {
                                    'Iteration': iteration['iteration'],
                                    'Open %': f"{iteration['open_percent']:.1f}",
                                    'Valve Cv': f"{iteration['valve_cv']:.2f}",
                                    'Req Cv': f"{iteration['cv_req']:.2f}",
                                    'Fp': f"{iteration.get('fp', 1.0):.4f}"
                                }
                                
                                if scenario['fluid_type'] == 'liquid':
                                    row['Fl'] = f"{iteration.get('fl_at_op', 0):.4f}"
                                    row['Re'] = f"{iteration.get('reynolds', 0):.0f}"
                                    row['Fr'] = f"{iteration.get('fr', 1.0):.4f}"
                                    row['ΔPmax'] = f"{iteration.get('dp_max', 0):.2f}"
                                    row['Choked'] = 'Yes' if iteration.get('is_choked', False) else 'No'
                                elif scenario['fluid_type'] == 'gas':
                                    row['Xt'] = f"{iteration.get('xt_at_op', 0):.4f}"
                                    row['x'] = f"{iteration.get('x_actual', 0):.4f}"
                                    row['x_crit'] = f"{iteration.get('x_crit', 0):.4f}"
                                    row['Y'] = f"{iteration.get('expansion_factor', 0):.4f}"
                            ...
                            if len(result['iteration_history']) > 1:
                                st.info(f"**Convergence:** {len(result['iteration_history'])-1} iterations required to reach final operating point.")
                        ...
                        with col1:
                            st.metric("Required Cv", f"{result['req_cv']:.2f}")
                        with col2:
                            st.metric("Operating Point", f"{result['op_point']:.1f}%")
                        with col3:
                            st.metric("Valve Cv", f"{selected_valve.get_cv_at_opening(result['op_point']):.2f}")
                        with col4:
