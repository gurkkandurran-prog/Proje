...
WATER_DENSITY_15C = 999.13
AIR_DENSITY_STD = 1.225  # kg/m³ at 15°C, 1 atm
...
# ========================
# FLUID LIBRARY WITH UPDATED SPECIFIC GRAVITY FUNCTIONS
# ========================
def calculate_specific_gravity_liquid(fluid_name: str, temp_c: float, pressure_bar: float) -> float:
    """
    Calculate specific gravity for liquids relative to water at 4°C
    SG = ρ_fluid(T,P) / ρ_water(4°C)
    """
    try:
        if fluid_name in FLUID_LIBRARY and FLUID_LIBRARY[fluid_name].get("coolprop_name"):
            # Get fluid density at given conditions
            fluid_density = calculate_density(fluid_name, temp_c, pressure_bar)
            # Water density at 4°C is reference
            return fluid_density / WATER_DENSITY_4C
    except:
        pass
...
def calculate_specific_gravity_gas(fluid_name: str, temp_c: float, pressure_bar: float) -> float:
    ...
    try:
        if fluid_name in FLUID_LIBRARY and FLUID_LIBRARY[fluid_name].get("coolprop_name"):
            # Calculate using molecular weight ratio (more accurate for gases)
            fluid_name_cp = FLUID_LIBRARY[fluid_name]["coolprop_name"]
            
            # Get molecular weights
            M_fluid = CP.PropsSI('M', fluid_name_cp)  # kg/kmol
            M_air = CP.PropsSI('M', 'air')  # kg/kmol
            
            # For ideal gases, SG = M_fluid / M_air
            return M_fluid / M_air
    except:
        pass
    ...
    try:
        if fluid_name in FLUID_LIBRARY and FLUID_LIBRARY[fluid_name].get("coolprop_name"):
            ...
            P_std = STANDARD_PRESSURE  # 1.01325 bar
            
            # Get fluid density at standard conditions
            fluid_density_std = calculate_density(fluid_name, 15, 1.01325)
            
            # Get air density at standard conditions
            air_density_std = calculate_density("air", 15, 1.01325)
            
            if air_density_std > 0:
                return fluid_density_std / air_density_std
    except:
        pass
...
FLUID_LIBRARY = {
    "Water": {
        ...
        "coolprop_name": "Water",
        "sg_default": 1.0,
        "sg_func": lambda t, p: calculate_specific_gravity_liquid("Water", t, p),
    ...
    "Steam": {
        "type": "steam",
        "coolprop_name": "Water",
        "sg_default": 0.6,  # Approximate for steam relative to air
        "sg_func": lambda t, p: 0.6,  # Steam uses different equations, not typical SG
        "visc_func": None,
    ...
    "20% Hydrogen + 80% Methane": {
        ...
        "sg_func": lambda t, p: calculate_specific_gravity_mixture(t, p, 0.2, 0.8, "Hydrogen", "Methane"),
        "visc_func": None,
    ...
    "Natural Gas": {
        ...
        "sg_func": lambda t, p: calculate_specific_gravity_gas("Methane", t, p) * 1.08,  # Adjust for natural gas
        "visc_func": None,
        "k_func": lambda t, p: calculate_specific_heat_ratio("Methane", t, p),
        "z_func": lambda t, p: calculate_compressibility_factor("Methane", t, p),
        "pv_func": None,
    ...
    "Octane": {
        ...
        "sg_default": 0.74,
        "sg_func": lambda t, p: calculate_specific_gravity_liquid("Octane", t, p),
    ...
    "JP8": {
        ...
        "sg_func": lambda t, p: calculate_specific_gravity_liquid("Decane", t, p) * 0.95,  # Adjust for JP8
    ...
    "JET A1": {
        ...
        "sg_func": lambda t, p: calculate_specific_gravity_liquid("Decane", t, p) * 0.94,  # Adjust for Jet A1
...
def calculate_specific_gravity_mixture(temp_c: float, press_bar: float, frac1: float, frac2: float, fluid1: str, fluid2: str) -> float:
    """
    Calculate specific gravity for gas mixtures
    SG_mixture = frac1 * SG1 + frac2 * SG2
    """
    try:
        if fluid1 in FLUID_LIBRARY and "sg_func" in FLUID_LIBRARY[fluid1]:
            sg1 = FLUID_LIBRARY[fluid1]["sg_func"](temp_c, press_bar)
        else:
            sg1 = FLUID_LIBRARY[fluid1]["sg_default"] if fluid1 in FLUID_LIBRARY else 1.0
        
        if fluid2 in FLUID_LIBRARY and "sg_func" in FLUID_LIBRARY[fluid2]:
            sg2 = FLUID_LIBRARY[fluid2]["sg_func"](temp_c, press_bar)
        else:
            sg2 = FLUID_LIBRARY[fluid2]["sg_default"] if fluid2 in FLUID_LIBRARY else 1.0
        
        return frac1 * sg1 + frac2 * sg2
    except:
        # Fallback to default mixture calculation
        return frac1 * FLUID_LIBRARY.get(fluid1, {}).get("sg_default", 1.0) + frac2 * FLUID_LIBRARY.get(fluid2, {}).get("sg_default", 1.0)
...
def calculate_density(fluid: str, temp_c: float, press_bar: float) -> float:
    ...
    if fluid == "water" or fluid == "Water":
        # Water density approximation (simplified)
        ...
        if T <= 4:
            ...
            return 958.4 * (100 / max(temp_c, 1))  # Rough extrapolation
    elif fluid == "air":
        # Ideal gas law for air: ρ = P * M / (R * T)
        R = 8.314462  # J/(mol·K)
...
def calculate_kinematic_viscosity(fluid: str, temp_c: float, press_bar: float) -> float:
    try:
        ...
        pass
    if fluid == "water" or fluid == "Water":
        # Water viscosity correlation (approximate)
        if temp_c <= 0:
            return 1.79
        elif temp_c <= 20:
            return 1.0
            ...
            return 0.28 * (100 / max(temp_c, 1))  # Rough extrapolation
    elif fluid == "propane":
...
def calculate_specific_heat_ratio(fluid: str, temp_c: float, press_bar: float) -> float:
    ...
    elif fluid == "steam" or fluid == "Steam":
...
def evaluate_valve_for_scenario(valve, scenario):
    ...
    if scenario["fluid_type"] == "liquid":
        if scenario.get('fluid_library') in FLUID_LIBRARY:
            ...
            # Calculate specific gravity dynamically
        ...
        if scenario.get('fluid_library') in FLUID_LIBRARY:
            ...
            scenario["k"] = fluid_data["k_func"](scenario["temp"], scenario["p1"])
            # Calculate specific gravity for steam (though not typically used in steam equations)
            if "sg_func" in fluid_data:
                scenario["sg"] = fluid_data["sg_func"](scenario["temp"], scenario["p1"])
...
def find_recommended_valve(scenarios):
    ...
    # Check if VALVE_DATABASE is empty
    if not VALVE_DATABASE:
        return None, []
        ...
        for scenario in scenarios:
            try:
                ...
                if result.get("status") == "red":
                    valve_score -= 100
                    is_suitable = False
                elif result.get("status") == "orange":
                    valve_score -= 50
                elif result.get("status") == "yellow":
                    valve_score -= 10
                else:
                    valve_score += 20
                    
                valve_score += max(0, 10 - abs(result.get("op_point", 0) - 50)/5)
            except Exception as e:
                # If evaluation fails, mark as unsuitable
                valve_results.append({
                    "op_point": 0,
                    "req_cv": 0,
                    "warning": f"Evaluation error: {str(e)[:50]}",
                    "status": "red"
                })
                valve_score -= 100
                is_suitable = False
    ...
    if best_valve is None and all_valve_results:
        all_valve_results.sort(key=lambda x: x.get("score", 0), reverse=True)
        best_valve = all_valve_results[0] if all_valve_results else None
    
    return best_valve, all_valve_results

# ========================
# FLUID PROPERTY UPDATE FUNCTIONS - UPDATED WITH SPECIFIC GRAVITY
# ========================
...
def main():
    ...
    if calculate_btn:
        ...
        try:
            ...
            st.session_state.results = {
                ...
                "all_valve_results": all_valve_results if all_valve_results else []
            }
        except Exception as e:
    ...
    with tab_results:
        if st.session_state.results:
            ...
            recommended_valve = results.get("recommended_valve")  # Use .get() to avoid KeyError
            
            # First, check if we have all_valve_results
            all_valve_results = results.get("all_valve_results", [])
            ...
            **Status colors**:
            ...
            # Get all_valve_results from the results, default to empty list
            all_valve_results = results.get("all_valve_results", [])
            if all_valve_results:
                # Sort the results by score
                all_valve_results = sorted(all_valve_results, key=lambda x: x.get("score", 0), reverse=True)
                ...
                for valve_result in all_valve_results:
                    all_valves_table_html += f'<tr><td>{valve_result.get("display_name", "N/A")}</td>'
                    for result in valve_result.get("results", []):
                        status_class = ""
                        warning = result.get("warning", "")
                        if "Insufficient" in warning:
                            status_class = "status-insufficient"
                        elif "High velocity" in warning:
                        ...
                        elif result.get("status") == "red":
                            status_class = "status-red"
                        all_valves_table_html += f'<td class="{status_class}">{result.get("status", "N/A")}</td>'
                    all_valves_table_html += f'<td>{valve_result.get("score", 0):.1f}</td></tr>'
                all_valves_table_html += "</tbody></table>"
                st.markdown(all_valves_table_html, unsafe_allow_html=True)
            else:
                st.info("No valve evaluation data available.")
        else:
            st.info("Please run the calculation first to see results.")
