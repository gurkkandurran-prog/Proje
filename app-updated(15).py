...
def scenario_input_form(scenario_num, scenario_data=None):
    ...
    with col2:
        if fluid_library != "Select Fluid Library...":
            ...
            st.text_input("Fluid Type", value=fluid_type.capitalize(), disabled=False, key=f"fluid_type_text_{scenario_num}")
        else:
            try:
    ...
    with col2:
        ...
        if fluid_type == "gas":
            # Show Z as disabled if fluid library is selected, else editable
            if fluid_library != "Select Fluid Library...":
                z = st.number_input(
                    ...
                    disabled=False  # Disabled for library fluids
                )
            else:
                z = st.number_input(
                    "Compressibility Factor (Z)", 
