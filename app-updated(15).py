...
def scenario_input_form(scenario_num, scenario_data=None):
    ...
    # Akışkan özelliklerini her zaman güncelle (basınç ve sıcaklık değiştiğinde de)
    if fluid_library != "Select Fluid Library...":
        fluid_data = FLUID_LIBRARY[fluid_library]
        scenario_data["fluid_type"] = fluid_data["type"]
        
        # Mevcut basınç ve sıcaklık değerlerini al
        p1 = scenario_data["p1"]
        temp = scenario_data["temp"]
        
        # Tüm akışkan özelliklerini güncelle
        ...
        elif fluid_data.get("rho_func"):
            # Yoğunluktan specific gravity hesapla
            if fluid_data["type"] == "liquid":
                scenario_data["sg"] = fluid_data["rho_func"](temp, p1) / 1000
            elif fluid_data["type"] == "gas":
                scenario_data["sg"] = fluid_data["rho_func"](temp, p1) / 1.225  # Havaya göre
    ...
    with col2:
        # Akışkan özelliklerini basınç ve sıcaklık değiştiğinde tekrar güncelle
        if fluid_library != "Select Fluid Library...":
            fluid_data = FLUID_LIBRARY[fluid_library]
            
            # Tüm akışkan özelliklerini güncelle (yeniden)
            if fluid_data.get("visc_func") and fluid_data["type"] == "liquid":
                scenario_data["visc"] = fluid_data["visc_func"](temp, p1)
            if fluid_data.get("k_func") and fluid_data["type"] in ["gas", "steam"]:
                scenario_data["k"] = fluid_data["k_func"](temp, p1)
            ...
            if fluid_data.get("sg") is not None:
                ...
                if fluid_data["type"] == "liquid":
                    ...
                    scenario_data["sg"] = fluid_data["rho_func"](temp, p1) / 1.225
