...
def scenario_input_form(scenario_num, scenario_data=None):
    ...
    with col2:
        ...
        prev_fluid_library = st.session_state.get(f"prev_fluid_library_{scenario_num}", None)
        prev_temp = st.session_state.get(f"prev_temp_{scenario_num}", None)
        prev_p1 = st.session_state.get(f"prev_p1_{scenario_num}", None)
        
        # Check if fluid library, temperature, or pressure has changed
        current_temp = scenario_data["temp"]
        current_p1 = scenario_data["p1"]
        
        fluid_changed = fluid_library != prev_fluid_library
        temp_changed = current_temp != prev_temp
        pressure_changed = current_p1 != prev_p1
        
        # Update fluid properties if any relevant parameter changed
        if fluid_changed or temp_changed or pressure_changed:
...
def main():
    ...
    if 'prev_fluid_library_1' not in st.session_state:
        st.session_state.prev_fluid_library_1 = None
    if 'prev_temp_1' not in st.session_state:
        st.session_state.prev_temp_1 = None
    if 'prev_p1_1' not in st.session_state:
        st.session_state.prev_p1_1 = None
    # Repeat for scenarios 2 and 3 if needed
    if 'prev_fluid_library_2' not in st.session_state:
        st.session_state.prev_fluid_library_2 = None
    if 'prev_temp_2' not in st.session_state:
        st.session_state.prev_temp_2 = None
    if 'prev_p1_2' not in st.session_state:
        st.session_state.prev_p1_2 = None
    if 'prev_fluid_library_3' not in st.session_state:
        st.session_state.prev_fluid_library_3 = None
    if 'prev_temp_3' not in st.session_state:
        st.session_state.prev_temp_3 = None
    if 'prev_p1_3' not in st.session_state:
        st.session_state.prev_p1_3 = None
