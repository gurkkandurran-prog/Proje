...
# ========================
def calculate_valve_velocity(scenario, valve, op_point, req_cv, details):
    """Calculate flow velocities at valve opening point - FIXED VERSION"""
    try:
        ...
        # Full bore area (inlet area)
        area_full = math.pi * (diameter_m / 2)**2  # in m²
        
        # Flow area at operating point (approximate using Cv ratio)
        # This is an approximation - in reality it depends on valve trim characteristics
        if cv_100 > 0:
            area_op = area_full * (cv_op / cv_100)
        else:
            area_op = area_full
        
        # Get the ACTUAL flow rate that would pass through the valve at current conditions
        # This is different from the scenario flow rate due to choking, viscosity, etc.
        if scenario["fluid_type"] == "liquid":
            # Use the flow rate that corresponds to the actual Cv and pressure drop
            N1 = CONSTANTS["N1"]["m³/h, bar"]
            actual_flow_m3h = cv_op * N1 * math.sqrt((scenario['p1'] - scenario['p2']) / scenario['sg'])
            flow_m3s = actual_flow_m3h / 3600  # Convert to m³/s
            
        elif scenario["fluid_type"] == "gas":
            # For gases, we need to use the actual volumetric flow at valve conditions
            N7 = CONSTANTS["N7"]["m³/h, bar, K (standard)"]
            x_actual = (scenario['p1'] - scenario['p2']) / scenario['p1']
            x_crit = details.get('x_crit', 0)
            ...
            if x_actual >= x_crit:
                ...
                y = 1 - x_actual / (3 * (scenario['k'] / 1.4) * details.get('xt_at_op', 0.5))
                x = x_actual
            
            # Calculate actual volumetric flow through valve
            actual_flow_m3h = cv_op * N7 * scenario['p1'] * y * math.sqrt(
                x / (scenario['sg'] * (scenario['temp'] + C_TO_K) * scenario['z']))
            
            # Convert to actual m³/s (already at valve conditions)
            flow_m3s = actual_flow_m3h / 3600
            
        else:  # steam
            # For steam, use mass flow converted to volumetric flow
            N6 = CONSTANTS["N6"]["kg/h, bar, kg/m³"]
            x_actual = (scenario['p1'] - scenario['p2']) / scenario['p1']
            x_crit = details.get('x_crit', 0)
            ...
            # Calculate mass flow through valve
            mass_flow_kg_h = cv_op * N6 * y * math.sqrt(x * scenario['p1'] * scenario['rho'])
            
            # Convert to volumetric flow at valve conditions
            volume_flow_m3h = mass_flow_kg_h / scenario['rho']  # m³/h
            flow_m3s = volume_flow_m3h / 3600  # m³/s
        ...
        max_inlet_velocity = VELOCITY_LIMITS.get(scenario["fluid_type"], 10)
        max_orifice_velocity = max_inlet_velocity * 2  # Orifice velocity can be higher
        
        if inlet_velocity > max_inlet_velocity:
            inlet_warning = f"High inlet velocity ({inlet_velocity:.1f} m/s) for {scenario['fluid_type']}! (max {max_inlet_velocity} m/s)"
        
        if orifice_velocity > max_orifice_velocity:
            orifice_warning = f"High orifice velocity ({orifice_velocity:.1f} m/s) for {scenario['fluid_type']}! (max {max_orifice_velocity} m/s)"
        
        # Combine warnings
        velocity_warning = ""
        if inlet_warning and orifice_warning:
            velocity_warning = f"{inlet_warning}; {orifice_warning}"
        elif inlet_warning:
            velocity_warning = inlet_warning
        elif orifice_warning:
            velocity_warning = orifice_warning
            
        return orifice_velocity, inlet_velocity, velocity_warning
        
    except Exception as e:
        # Fallback in case of calculation error
        return 0, 0, f"Velocity calculation error: {str(e)}"
...
def evaluate_valve_for_scenario(valve, scenario):
    ...
    # Calculate velocity - PASS THE REQUIRED PARAMETERS
    orifice_velocity, inlet_velocity, velocity_warning = calculate_valve_velocity(
    scenario, valve, open_percent, cv_req, details
    )
