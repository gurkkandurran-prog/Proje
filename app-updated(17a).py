...
# ========================
# UPDATED CV_LIQUID FUNCTION WITH ITERATIVE Fp CALCULATION
# ========================
...
# ========================
# UPDATED CV_GAS FUNCTION WITH ITERATIVE Fp CALCULATION
# ========================
...
# ========================
# UPDATED CV_STEAM FUNCTION WITH ITERATIVE Fp CALCULATION
# ========================
...
# ========================
# UPDATED RECOMMENDED VALVE LOGIC WITH ITERATIVE Fp CALCULATION
# ========================
def evaluate_valve_for_scenario(valve, scenario):
    """
    Evaluate valve for scenario with iterative Fp calculation using Cv at operating point
    """
    ...
    max_iterations = 10
    tolerance = 1.0  # 1% tolerance for opening convergence
    prev_opening = 0
    ...
    for iteration in range(max_iterations):
        ...
        if scenario["fluid_type"] == "liquid":
            ...
            # For gases, we may need to recalculate xTP if pipe diameter is different
            if abs(pipe_d - valve_d) > 0.01:
                xt_at_op = calculate_x_tp(valve, valve_d, pipe_d, fp, cv_at_op, xt_at_op)
            ...
            # For steam, we may need to recalculate xTP if pipe diameter is different
            if abs(pipe_d - valve_d) > 0.01:
                xt_at_op = calculate_x_tp(valve, valve_d, pipe_d, fp, cv_at_op, xt_at_op)
        ...
        if abs(new_opening - prev_opening) < tolerance:
            opening = new_opening
            converged = True
            break
    ...
    # After iteration, get final values
    cv_at_op = valve.get_cv_at_opening(opening)
    fl_at_op = valve.get_fl_at_opening(opening)
    xt_at_op = valve.get_xt_at_opening(opening)
    ...
    # Final Cv calculation with converged values
    if scenario["fluid_type"] == "liquid":
        cv_req, details = cv_liquid(
            flow=flow_std,
            p1=scenario["p1"],
    ...
    cv_valve = valve.get_cv_at_opening(opening)
    
    warn = ""
    # Check for insufficient capacity
    if opening >= 100 and cv_valve < cv_req:
                    ...
                    with st.expander(f"Detailed Calculations for {scenario['name']}"):
                        ...
                        if result.get('iterations'):
                            with st.expander("Iteration Details"):
                                st.markdown("**Fp Calculation Iterations:**")
                                ...
                                if result.get('converged'):
                                    ...
                                    st.warning(f"Did not fully converge after {len(result['iterations'])} iterations")
