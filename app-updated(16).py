...
# ========================
# REACTIVE FLUID PROPERTY UPDATES
# ========================
def update_fluid_properties(scenario_data, fluid_library, temp, p1):
    """Update all fluid properties reactively based on current conditions"""
    if fluid_library != "Select Fluid Library...":
        ...
        if fluid_data["type"] == "liquid":
            ...
            if fluid_data.get("rho_func"):
                scenario_data["sg"] = fluid_data["rho_func"](temp, p1) / WATER_DENSITY_4C
            elif fluid_data.get("sg") is not None:
            ...
            if fluid_data.get("rho_func"):
                # Calculate density for reference (not used in gas Cv calculation but good to have)
...
def display_fluid_properties_in_results(scenario, result):
    """Display fluid properties in the results section with proper formatting"""
    st.subheader("Fluid Properties")
    col1, col2 = st.columns(2)
    
    with col1:
        st.markdown(f"**Fluid:** {scenario.get('fluid_library', 'Custom')}")
        st.markdown(f"**Type:** {scenario['fluid_type'].title()}")
        st.markdown(f"**Temperature:** {scenario['temp']}°C")
        st.markdown(f"**Inlet Pressure:** {scenario['p1']:.2f} bar a")
        
    with col2:
        if scenario["fluid_type"] == "liquid":
            st.markdown(f"**Specific Gravity:** {scenario['sg']:.3f}")
            st.markdown(f"**Viscosity:** {scenario['visc']:.2f} cSt")
...
def scenario_input_form(scenario_num, scenario_data=None):
    # Initialize session state for tracking fluid changes
    if f'prev_fluid_library_{scenario_num}' not in st.session_state:
        st.session_state[f'prev_fluid_library_{scenario_num}'] = "Select Fluid Library..."
    if f'prev_temp_{scenario_num}' not in st.session_state:
        st.session_state[f'prev_temp_{scenario_num}'] = 20.0
    if f'prev_p1_{scenario_num}' not in st.session_state:
        st.session_state[f'prev_p1_{scenario_num}'] = 10.0
    ...
    with col2:
        ...
        current_temp = scenario_data["temp"]
        current_p1 = scenario_data["p1"]
        
        # Check if fluid library, temperature, or pressure has changed
        fluid_changed = fluid_library != st.session_state[f'prev_fluid_library_{scenario_num}']
        temp_changed = current_temp != st.session_state[f'prev_temp_{scenario_num}']
        pressure_changed = current_p1 != st.session_state[f'prev_p1_{scenario_num}']
        
        # Update fluid properties if any relevant parameter changed and a fluid is selected
        if (fluid_changed or temp_changed or pressure_changed) and fluid_library != "Select Fluid Library...":
            scenario_data = update_fluid_properties(scenario_data, fluid_library, current_temp, current_p1)
            
            # Store current values for next comparison
            st.session_state[f'prev_fluid_library_{scenario_num}'] = fluid_library
            st.session_state[f'prev_temp_{scenario_num}'] = current_temp
            st.session_state[f'prev_p1_{scenario_num}'] = current_p1
        
        # Display fluid type (read-only when using fluid library)
        if fluid_library != "Select Fluid Library...":
            fluid_data = FLUID_LIBRARY[fluid_library]
    ...
    with col2:
        # Update scenario data with current values
        scenario_data["p1"] = p1
        scenario_data["temp"] = temp
        
        # Fluid properties section with reactive updates
...
def main():
    ...
    # Initialize fluid tracking for each scenario
    for i in range(1, 4):
        if f'prev_fluid_library_{i}' not in st.session_state:
            st.session_state[f'prev_fluid_library_{i}'] = "Select Fluid Library..."
        if f'prev_temp_{i}' not in st.session_state:
            st.session_state[f'prev_temp_{i}'] = 20.0
        if f'prev_p1_{i}' not in st.session_state:
            st.session_state[f'prev_p1_{i}'] = 10.0
    ...
    with tab_results:
        if st.session_state.results:
            ...
            if recommended_valve:
                ...
                for i, scenario in enumerate(scenarios):
                    ...
                    with st.container():
                        ...
                        cols[2].metric("Theo Cv", f"{result['theoretical_cv']:.1f}")
                        cols[3].metric("Opening", f"{result['op_point']:.1f}%")
                        cols[4].metric("Actual Cv", f"{actual_cv:.1f}")
                        cols[5].metric("Margin", f"{result['margin']:.1f}%")
                        cols[6].metric("Orifice Vel", f"{result['orifice_velocity']:.1f} m/s")
                        cols[7].markdown(f"**Status:** {warn_text}")
                        st.markdown("</div>", unsafe_allow_html=True)
            
            st.subheader("Selected Valve Results")
            for i, scenario in enumerate(scenarios):
            ...
            # Generate and display Cv curve plot
            op_points = [r["op_point"] for r in selected_valve_results]
            req_cvs = [r["req_cv"] for r in selected_valve_results]
            theoretical_cvs = [r["theoretical_cv"] for r in selected_valve_results]
            scenario_names = [s["name"] for s in scenarios]
            
            cv_plot = plot_cv_curve(selected_valve, op_points, req_cvs, theoretical_cvs, scenario_names)
            st.plotly_chart(cv_plot, use_container_width=True)
            
            # Generate and display Flow vs DP plot for first scenario
            if scenarios:
                flow_dp_plot = generate_flow_vs_dp_graph(
                    scenarios[0], selected_valve, op_points[0], selected_valve_results[0]["details"], req_cvs[0]
                )
                st.plotly_chart(flow_dp_plot, use_container_width=True)
            
            # Alternative calculation results for gas and steam
            for i, scenario in enumerate(scenarios):
                if scenario["fluid_type"] in ["gas", "steam"]:
                    result = selected_valve_results[i]
                    details = result["details"]
                    if "cv_alternative" in details and details["cv_alternative"] > 0:
                        st.info(f"Alternative calculation for {scenario['name']}: {details['alternative_method']}, Cv = {details['cv_alternative']:.1f}")
            
            # Display fluid properties in results
            if scenarios:
                display_fluid_properties_in_results(scenarios[0], selected_valve_results[0])
        
        else:
            st.info("Click 'Calculate Opening' to see results.")
            ...
            try:
                with st.spinner("Generating PDF Report..."):
                    ...
                    pdf_bytes = generate_pdf_report(
                        scenarios, selected_valve, op_points, req_cvs, warnings, cavitation_info,
                        plot_bytes, flow_dp_plot_bytes, st.session_state.logo_bytes, st.session_state.logo_type
                    )
    ...
    if st.session_state.show_3d_viewer:
        valve_display_name = get_valve_display_name(selected_valve)
        model_url = VALVE_MODELS.get(valve_display_name, "")
        if model_url:
            st.markdown("<div class='modal-backdrop'></div>", unsafe_allow_html=True)
            st.markdown("<div class='simulation-modal'>", unsafe_allow_html=True)
            st.subheader(f"3D Model: {valve_display_name}")
            valve_3d_viewer(valve_display_name, model_url)
    ...
    if st.session_state.show_simulation:
        valve_display_name = get_valve_display_name(selected_valve)
        sim_url = get_simulation_image(valve_display_name)
        st.markdown("<div class='modal-backdrop'></div>", unsafe_allow_html=True)
        st.markdown("<div class='simulation-modal'>", unsafe_allow_html=True)
        st.subheader(f"CFD Simulation Results: {valve_display_name}")
