...
def calculate_piping_factor_fp(valve_d_inch: float, upstream_pipe_d_inch: float, downstream_pipe_d_inch: float, cv_100: float) -> float:
    """
    Calculate piping geometry factor Fp with separate upstream and downstream pipe diameters
    """
    if (abs(upstream_pipe_d_inch - valve_d_inch) < 0.01 and 
        abs(downstream_pipe_d_inch - valve_d_inch) < 0.01):
        return 1.0
    
    # Calculate sum of loss coefficients
    # For upstream reducer/expander
    if upstream_pipe_d_inch > valve_d_inch:  # Reducer
        K1 = 0.5 * (1 - (valve_d_inch/upstream_pipe_d_inch)**2)**2
    else:  # Expander
        K1 = 0.0  # Expander effect is neglected in standard calculation
    
    # For downstream reducer/expander
    if downstream_pipe_d_inch > valve_d_inch:  # Reducer
        K2 = 1.0 * (1 - (valve_d_inch/downstream_pipe_d_inch)**2)**2
    else:  # Expander
        K2 = 0.0  # Expander effect is neglected in standard calculation
    
    # Bernoulli coefficients
    KB1 = 1 - (valve_d_inch/upstream_pipe_d_inch)**4 if upstream_pipe_d_inch > 0 else 0
    KB2 = 1 - (valve_d_inch/downstream_pipe_d_inch)**4 if downstream_pipe_d_inch > 0 else 0
...
def calculate_flp(valve, valve_d_inch: float, upstream_pipe_d_inch: float, downstream_pipe_d_inch: float, cv_100: float) -> float:
    """
    Calculate combined liquid pressure recovery factor with piping geometry
    """
    if (abs(upstream_pipe_d_inch - valve_d_inch) < 0.01 and 
        abs(downstream_pipe_d_inch - valve_d_inch) < 0.01):
        return valve.get_fl_at_opening(100)
    
    # Calculate inlet loss coefficient Ki
    if upstream_pipe_d_inch > valve_d_inch:  # Reducer
        K1 = 0.5 * (1 - (valve_d_inch/upstream_pipe_d_inch)**2)**2
        KB1 = 1 - (valve_d_inch/upstream_pipe_d_inch)**4
    else:  # Expander
        K1 = 0.0
        KB1 = 0.0
...
def calculate_x_tp(valve, valve_d_inch: float, upstream_pipe_d_inch: float, downstream_pipe_d_inch: float, Fp: float) -> float:
    """
    Calculate pressure drop ratio factor with piping geometry
    """
    if (abs(upstream_pipe_d_inch - valve_d_inch) < 0.01 and 
        abs(downstream_pipe_d_inch - valve_d_inch) < 0.01):
...
def generate_simple_pdf_report(scenarios, valve, op_points, req_cvs, warnings, cavitation_info,
    ...
    try:
        ...
        for i, scenario in enumerate(scenarios):
            ...
            pdf.add_key_value("Upstream Pipe Diameter", f"{scenario['upstream_pipe_d']} inch")
            pdf.add_key_value("Downstream Pipe Diameter", f"{scenario['downstream_pipe_d']} inch")
...
def evaluate_valve_for_scenario(valve, scenario):
    """
    Evaluate valve performance for a given scenario with iterative calculation
    of piping factors Fp, FLP, and xTP until convergence (99% or 10 iterations max)
    """
    # Determine pipe diameters based on selection
    use_valve_size = scenario.get("use_valve_size", True)
    if use_valve_size:
        upstream_pipe_d = valve.diameter
        downstream_pipe_d = valve.diameter
    else:
        upstream_pipe_d = scenario["upstream_pipe_d"]
        downstream_pipe_d = scenario["downstream_pipe_d"]
    ...
    prev_op_point = 0
    op_point = 50  # Start with 50% as initial guess
    
    # Calculate initial piping factors using rated values (100% opening)
    Fp = calculate_piping_factor_fp(valve_d, upstream_pipe_d, downstream_pipe_d, cv_100)
    FLP = calculate_flp(valve, valve_d, upstream_pipe_d, downstream_pipe_d, cv_100)
    xTP = calculate_x_tp(valve, valve_d, upstream_pipe_d, downstream_pipe_d, Fp)
    
    # Iterate until convergence
    for iteration in range(max_iterations):
        # Calculate fluid properties at current iteration
        if scenario["fluid_type"] == "liquid":
            # Get Fl at current operating point
            fl_at_op = valve.get_fl_at_opening(op_point)
            ...
            # Find new operating point based on required Cv
            new_op_point = 10
            ...
            xt_at_op = valve.get_xt_at_opening(op_point)
            # Use xTP if piping geometry is different
            if not (abs(upstream_pipe_d - valve_d) < 0.01 and abs(downstream_pipe_d - valve_d) < 0.01):
                xt_at_op = xTP
        ...
        if abs(new_op_point - prev_op_point) / max(new_op_point, 1) <= tolerance:
        ...
        # Recalculate piping factors with current operating point characteristics
        cv_current = valve.get_cv_at_opening(op_point)
        Fp = calculate_piping_factor_fp(valve_d, upstream_pipe_d, downstream_pipe_d, cv_current)
        FLP = calculate_flp(valve, valve_d, upstream_pipe_d, downstream_pipe_d, cv_current)
        xTP = calculate_x_tp(valve, valve_d, upstream_pipe_d, downstream_pipe_d, Fp)
    ...
    orifice_velocity, inlet_velocity, inlet_warning = calculate_valve_velocity(scenario, valve, op_point)
    
    # Final valve Cv at operating point
    cv_valve = valve.get_cv_at_opening(op_point)
    ...
    if op_point >= 100 and cv_valve < cv_req:
...
def scenario_input_form(scenario_num, scenario_data=None):
    ...
    if scenario_data is None:
        ...
        # Handle backward compatibility for existing scenarios
        if "pipe_d" in scenario_data and "upstream_pipe_d" not in scenario_data:
            scenario_data["upstream_pipe_d"] = scenario_data["pipe_d"]
            scenario_data["downstream_pipe_d"] = scenario_data["pipe_d"]
    ...
    with col2:
        ...
        if not use_valve_size:
            ...
            with col_up:
                upstream_pipe_d = st.number_input(
                    "Upstream Pipe Diameter (inch)", 
                    min_value=0.1, 
                    max_value=100.0, 
                    value=scenario_data["upstream_pipe_d"], 
                    step=0.1,
                    key=f"upstream_pipe_d_{scenario_num}"
                )
            with col_down:
                downstream_pipe_d = st.number_input(
                    "Downstream Pipe Diameter (inch)", 
                    min_value=0.1, 
                    max_value=100.0, 
                    value=scenario_data["downstream_pipe_d"], 
                    step=0.1,
                    key=f"downstream_pipe_d_{scenario_num}"
                )
        else:
            upstream_pipe_d = scenario_data["upstream_pipe_d"]
            downstream_pipe_d = scenario_data["downstream_pipe_d"]
    ...
    return {
        ...
        "upstream_pipe_d": upstream_pipe_d,
        "downstream_pipe_d": downstream_pipe_d,
                ...
                with st.container():
                    ...
                    with st.expander(f"Detailed Calculations for {scenario['name']}"):
                        ...
                        with col1:
                            ...
                            # Display FLP and xTP if available
                            if 'flp' in result:
                                st.markdown(f"**FLP (Combined Factor):** {result['flp']:.4f}")
                            if 'xtp' in result:
                                st.markdown(f"**xTP (Pressure Drop Ratio):** {result['xtp']:.4f}")
                        ...
                        # Pipe diameters
                        st.markdown(f"**Upstream Pipe Diameter:** {scenario['upstream_pipe_d']} inch")
                        st.markdown(f"**Downstream Pipe Diameter:** {scenario['downstream_pipe_d']} inch")
